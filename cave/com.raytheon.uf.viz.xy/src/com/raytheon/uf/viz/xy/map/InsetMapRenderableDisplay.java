...
public class InsetMapRenderableDisplay extends PlainMapRenderableDisplay
    ...
    private static final int DEFAULT_WIDTH = 2_000_000;
    ...
    public InsetMapRenderableDisplay() {
        super();
        this.listener = new InsetRenderableDisplayListener();
    ...
    public void setParentDisplay(IRenderableDisplay parentDisplay) {
        ...
        for (ResourcePair rp : parentDisplay.getDescriptor()
                .getResourceList()) {
            addResource(rp);
        }
        this.parentDisplay.getDescriptor().getResourceList()
        ...
        synchronized (resources) {
            if (recalc && !resources.isEmpty()) {
                ...
                if (!allInitialized()) {
                    ...
                    recalc = false;
                    calculateProjection(paintProps);
                }
            }
    ...
    private void calculateProjection(PaintProperties paintProps)
        ...
        if (coords.length == 1) {
            ...
            if (stereo) {
                crs = MapUtil.constructStereographic(MapUtil.AWIPS_EARTH_RADIUS,
                        MapUtil.AWIPS_EARTH_RADIUS, p.getY(), p.getX());
            } else {
            ...
            try {
                ...
                Geometry geom2 = JTS.transform(geom, transform);
                Point p2 = geom2.getCentroid();
                Point p2InLL = (Point) JTS.transform(p2, transform.inverse());
                p = p2InLL;
                width = geom2.getEnvelopeInternal().getWidth();
                height = geom2.getEnvelopeInternal().getHeight();
            } catch (TransformException | FactoryException e) {
                statusHandler.warn(
                        "Error calculating better width and height of maps in inset pane",
                        e);
            }
            if (width > height) {
     ...
     * @throws VizException
     */
    private void scaleAllToClientArea(GridGeometry2D geom2D)
            throws VizException {
        /*
        ...
        synchronized (SCALE_LOCK) {
            /*
             * Scale all insets that have the same inset descriptor type and
             * main descriptor type as us.
             */
            IDescriptor myMainDesc = parentDisplay.getDescriptor();
            IDescriptor myInsetDesc = getDescriptor();
            for (IPane pane : getContainer().getPanes()) {
                IDisplayPane insetCanvas = pane.getCanvas(CanvasType.INSET);
                if (insetCanvas != null
                        && myInsetDesc.isCompatible(insetCanvas.getDescriptor())
                        && myMainDesc.isCompatible(
                                pane.getMainCanvas().getDescriptor())) {
                    IMapDescriptor md = (IMapDescriptor) insetCanvas
                            .getDescriptor();
                    md.setGridGeometry(geom2D);
                    insetCanvas.getRenderableDisplay()
    ...
    protected class InsetRenderableDisplayListener
            extends RenderableDisplayListener {

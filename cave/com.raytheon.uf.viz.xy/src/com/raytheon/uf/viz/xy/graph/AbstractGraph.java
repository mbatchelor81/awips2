...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    protected void addAxis(IAxis axis) {
        ...
        shape.addLineSegment(
                new double[][] { { coords[0].x, coords[0].y, coords[0].z },
    ...
    protected void redrawGraph() {
        ...
        for (IAxis axis : new IAxis[] { minXAxis, maxXAxis, minYAxis,
                maxYAxis }) {
            addAxis(axis);
        }
    ...
    protected static void normalizeAxis(double[] minMax) {
        ...
        // if min == max, adjust them out a bit to avoid a 0 range
        if (min == max) {
            double delta = 0.1;
            if (Math.abs(min) > 0) {
                delta = determineIncrement(Math.abs(min));
            }
    ...
    protected void paintYTitle(IGraphicsTarget target,
        ...
        int height = target.getStringsBounds(titleString, "H")
                .getBounds().height * 2;
    ...
    protected void paintDataTimeUnits(IGraphicsTarget target,
        ...
        List<DrawableString> strings = new ArrayList<>(xLabels.size());
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
            ...
            if (loc.y + labelsHeight > paintProps.getView().getExtent()
                    .getMaxY()) {
                loc = new Coordinate(loc.x,
                        paintProps.getView().getExtent().getMaxY()
                                - labelsHeight,
                        loc.z);
            }
    ...
    protected void paintHeightUnits(IGraphicsTarget target,
        ...
        List<DrawableString> labels = new ArrayList<>(xAxes.length * 2);
    ...
    protected void createHeightAxis(HeightScale heightScale, double zoomLevel) {
        ...
        if (heightScale.getLabels() == null) {
            double range = Math
                    .abs(heightScale.getMaxVal() - heightScale.getMinVal());
            double inc = determineIncrement(range);
            inc /= zoomLevel;
            ...
            for (int i = 1; i < labels.size(); i++) {
                ...
                for (int j = 1; j <= mult; j++) {
                    xAxes[(i - 1) * mult + j]
                            .setDiscreteValue(lastVal + inc * j);
                }
                lastVal = val;
            }
        }

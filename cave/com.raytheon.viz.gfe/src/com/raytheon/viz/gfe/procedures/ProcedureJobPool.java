...
public class ProcedureJobPool {
    ...
    protected LinkedBlockingQueue<ProcedureRequest> workQueue = new LinkedBlockingQueue<>();
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    protected class ProcedureJob extends Job {
        ...
        protected IStatus run(IProgressMonitor monitor) {
            try {
                python = new ProcedureScriptFactory(dataMgr)
                        .createPythonScript();
        ...
        protected void processRequest(ProcedureRequest request) {
            ...
            try {
                ...
                statusHandler.handle(Priority.PROBLEM,
                        "Error running procedure " + request.getProcedureName(),
                        t);
                retVal = t;
            } finally {
                dataMgr.getEditActionProcessor()
                        .wrapUpExecute(request.getPreview(), false);
        ...
        private void execute(ProcedureController controller,
            ...
            try {
                List<String> argNames = controller
                        .getMethodArguments(procedureName, "execute");
        ...
        private Map<String, Object> getArgValues(List<String> args,
            ...
            for (String arg : args) {
                if ("varDict".equals(arg)) {
                    argValueMap.put("varDict", null);
                } else if ("editArea".equals(arg)) {
                    argValueMap.put("editArea", refSet);
                } else if ("timeRange".equals(arg)) {

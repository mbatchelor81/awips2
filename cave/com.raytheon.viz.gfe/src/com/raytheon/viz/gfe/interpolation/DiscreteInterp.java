 ...
 *  linearly interpolated intermediate positions of centers of areas); and
 ...
 * in interpolated areas, without gaps or overlaps appearing;
 * 2. recognizing entirely embedded wxs, one inside others, and maintaining
 * that relation at all interpolated areas without gaps or overlaps;
 ...
 * may have DIFFERENT byte grids (keys). This is to preserve some programming
 * flexibility(!) but requires the confusion to be unscrambled here before
 * interpolation can take place. Working grids based on the input byte grids
...
public class DiscreteInterp extends Interp {
    ...
    // used to control changes in shapes of areas.
    ...
    public DiscreteInterp(List<IGridSlice> dataslices, int[] baseDataIndices,
        ...
        // limits.
        _gaControl = new Grid2DFloat(_numSectors, 2);
        _faControl = new Grid2DFloat(_numSectors, 2);
    }
    ...
    public IGridSlice interpolate(int index) {
        ...
        DiscreteGridSlice baseData2 = (DiscreteGridSlice) getGridSlice(after);
        ...
        // test output only:
        // cout<<" interpResult grid "<<"at fraction="<<fraction<<std::endl;
        // cout<<interpResult<<std::endl;

        // Create the new GridSlice from the grid of interpolated indexes,
        // the associated List<Discretekey> _allKeys, and the
        ...
        IGridSlice newGS = new DiscreteGridSlice(_gridTimes.get(index),
                gridParmInfo, new GridDataHistory[] { gdh }, ptIndexes,
                _allKeys.toArray(new DiscreteKey[_allKeys.size()]));
     ...
     * @param workGrid2
     */
    private void unscrambleWxTypes(DiscreteGridSlice baseData1,
            DiscreteGridSlice baseData2, Grid2DInteger workGrid1,
            Grid2DInteger workGrid2) {
        // extract the byte index grids for keys
        Grid2DByte grid1 = baseData1.getDiscreteGrid();
        ...
        for (j = 0; j < keys2.length; j++) {
            ...
            // see if it matches any key in keys1
            boolean match = false;
            for (i = 0; i < keys1.length; i++) {
                if (keys2[j].equals(keys1[i])) {
                    match = true;
                    break;
                }
            }
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
     ...
     *   how many grid points found of this wx type in this grid
     *   this weather's area limits: max x, max y, min x, min y.
     *   boolean if contiguous to any other type,
     ...
     * in first position of its row and column.
     * Upper right side from diagonal indicates contiguity in first input grid;
    ...
    private void weatherInterpolator(int wxValue, Grid2DInteger newGrid,
        ...
        for (int i = 0; i < (2 * _xDim); i++) {
            for (int j = 0; j < (2 * _yDim); j++) {
                ...
                upIndex = sector + 1;
                if (upIndex >= _numSectors) {
                    upIndex = 0;
                }

                dnIndex = sector - 1;
                if (dnIndex < 0) {
                    dnIndex = _numSectors - 1;

...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    private void createStatusControls() {
        ...
        for (AbstractSbuTask task : tasks) {
            ...
            Label statusIcon = new Label(statusGroup,
                    SWT.HORIZONTAL | SWT.CENTER);
            statusIcon.setLayoutData(
                    new GridData(SWT.FILL, SWT.DEFAULT, true, false));
            StatusIcons icon = StatusIcons.NOT_STARTED;
            statusIcon.setForeground(
                    statusIcon.getDisplay().getSystemColor(icon.getColor()));
    ...
    public boolean close() {
        if (executor.isAlive()) {
            MessageDialog.openWarning(getShell(),
                    this.site + " Service Backup Status",
    ...
    private void updateStatus(final AbstractSbuTask task) {
        ...
        if (labels != null) {
            ...
            case IN_PROGRESS:
                ...
                statusTime = new Date(
                        System.currentTimeMillis() - statusTime.getTime());
                break;
            case SUCCESS:

...
=======
        ...
        for tr in trList:
            CloudBaseRH = self.newGrid(MaxCloudBase)
            sounding = self.makeNumericSounding(
                databaseID, "rh", self.levels, tr, noDataError=0
            )
            if sounding is not None:
                gh_c, rh_c = sounding
                for i in range(rh_c.shape[0]):
...
=======
    ...
    def copyNBMCeilingToCBP(self, sourceDBID, timeRange):
        """
        Copy CBP from Ceiling, if it's defined.
        """
        sourceWeName = "Ceiling"
        targetWeName = "CloudBasePrimary"
        sourceInv = self.getWEInventory(sourceDBID, sourceWeName, "SFC", timeRange)
        for tr in sourceInv:
            sourceGrid = self.getGrids(sourceDBID, sourceWeName, "SFC", tr)
            targetGrid = self.getGrids(self.mutableID(), targetWeName, "SFC", tr)
            if targetGrid is None:
                targetGrid = self.newGrid(250.0)
            mask = sourceGrid < 25000.0
            # Poke in the values and convert to 100s of feet
            targetGrid[mask] = sourceGrid[mask] / 100.0
            targetGrid.clip(0.0, 250.0, targetGrid)
...
=======
            ...
            # defined.
            if (
                selectedDBID.modelName() in self._NBMSources
                and "CloudBasePrimary" in supportedWEs
            ):
                if self._config.get("UseNBMCeiling", "No") == "Yes":
                    self.copyNBMCeilingToCBP(selectedDBID, genTimeRange)

            self.interpolateCmd(
                self._genElementsSelected,
                genTimeRange,
                "GAPS",
                "SYNC",
                interval=1,
            ...
            # Populate the convective elements if needed.
            selectedDBID = self._dbIDDict[self._selectedConSource][0]
            # See if the Previous Version was selected
            if (
                self._altCBVersionButtonState == "Previous Version"
                and len(self._dbIDDict[self._selectedConSource]) > 1
            ):
                selectedDBID = self._dbIDDict[self._selectedConSource][1]

            if (self._selectedConSource == "CONSShort") & (
                self._selectedConAlgorithm == "CloudBaseRH"
            ):
                self.statusBarMsg(
...
=======
                ...
                self.deleteCmd(
                    [self._conElementSelected], genTimeRange
                )  # remove the grids first
                for grid, tr in cloudBaseGrids:
                    self.createGrid(
                        self.mutableID(), self._conElementSelected, "SCALAR", grid, tr
                    )
                self._activeLabel.config(text="           ")
            else:
                self.deleteCmd(
                ...
                copyFromWE = self._selectedConAlgorithm
                copyCount = self.copyGridsFromOtherElement(
                    selectedDBID, copyFromWE, self._conElementSelected, genTimeRange
                )
                if copyCount == 0:
                    self.statusBarMsg(
                        "No grids found for "
                        + selectedDBID.modelName()
                        + " "
                        + copyFromWE,
                        "S",
                    )
...
=======
            ...
            self._genElementDict[weName] = tk.Checkbutton(elementFrame, text=weName)

            self._genElementDict[weName].grid(row=row, sticky=tk.W)
            self._genElementDict[weName].bind("<ButtonRelease-1>", cbHandler)
            self._genElementDict[weName].bind("<ButtonRelease-2>", cbHandler)
            if weName in copy.deepcopy(self._config["genElementDefaultList"]):
                self._genElementDict[weName].select()
...
=======
            self._prevCurButtonDict[buttonName] = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.prevCurSelected,
                variable=self._selectedPrevCur,
                value=buttonName,
            )
            self._prevCurButtonDict[buttonName].grid(row=row, sticky=tk.W)
...
=======
        ...
        for weName in self._genElementDict:
            ...
            if weName in self._modelWEDict[self._selectedGenSource]:
                self._genElementDict[weName].config(state=tk.NORMAL)
            else:  # not available for this model so deselect and remove from selected list
...
=======
            self._genSourceButtonDict[buttonName] = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.genSourceButtonClicked,
                variable=self._genSourceButtonSelected,
                value=buttonName,
            )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            self._timeButtonDict[buttonName] = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.timeButtonSelected,
                variable=self._timeButtonSelected,
                value=buttonName,
            )
        ...
        # Make the time frame, time scales and time labels
        timeScaleFrame = tk.Frame(
            self._genTimeFrame, relief=tk.GROOVE, bd=3, bg=self._bgColor
        )
        timeScaleFrame.grid(row=1, column=1, sticky=tk.E + tk.W, columnspan=2)
...
=======
    def altCBVersionButtonSelected(self, event):
        if self._altCBVersionButtonState == "Current Version":
            ...
            self._altCBVersionButton.config(text="Current Version")
            self._altCBVersionButtonState = "Current Version"
...
=======
        ...
        # Add the Current/Previous Version button
        buttonName = "Current Version"

        def cbHandler(event, self=self):
            return self.altCBVersionButtonSelected(event)
        ...
        self._altCBVersionButtonState = "Current Version"
        self._altCBVersionButton = tk.Button(
            conFrame,
            text=buttonName,
            font=font,
            height=1,
        ...
        self._altCBVersionButton.grid(row=0, column=0, columnspan=4)

        self._altCBVersionButton.bind("<ButtonRelease-1>", cbHandler)
...
=======
            self._conSourceButtonDict[buttonName] = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.conSourceButtonClicked,
                variable=self._conSourceButtonSelected,
                value=buttonName,
            )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            self._conElementDict[buttonName] = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.conElementSelected,
                variable=self._conElementStringVar,
                value=buttonName,
            )
            self._conElementDict[buttonName].grid(row=row, sticky=tk.W)
...
=======
        ...
        self._genElementsSelected = copy.deepcopy(self._config["genElementDefaultList"])

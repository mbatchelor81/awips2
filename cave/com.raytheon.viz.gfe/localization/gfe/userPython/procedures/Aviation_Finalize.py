...
=======
# Aviation_Finalize - Version 20200611
#
...
=======
            trList = self.getWEInventory(self.mutableID(), weName, selectedTR)
            weDict[weName] = {}

            cumParm = self.cumulativeParm(weName, self.mutableID())

            for tr in trList:

                grid = self.getGrids(self.mutableID(), weName, "SFC", tr)
                if tr.duration() == 3600:
                    weDict[weName][tr] = grid, emptyMask
                    continue
...
=======
                if (
                    subParts[1] == wxType
                    and subParts[2] == wxIntens
                    or subParts[1] == wxType
                    and wxIntens is None
                ):
                    wxIndex = self.getIndex(wxKey, wxKeys)
                    gridMask |= byteGrid == wxIndex
...
=======
        ...
        for subKey in subKeys:
            ...
            if wxType == wxTypeToReplace:  # Replace this subKey
                newWxKey += ":".join([newWxCov, newWxType, newWxIntens]) + ":<NoVis>:"
            else:  # Add to the existing key
                newWxKey += subKey

        return newWxKey

    # This method possibly obsolete
    def removeWxType(self, wxGrid, removeWxType):
...
=======
            wxParts = self.wxCovTypeIntensMatchType(
                wxKey, newType
            )  # Returns None if not the same wx type
            if wxParts is not None:  # We found matching wxTypes somewhere in the full key
                if len(wxKey.split("^")) == 1:  # only one subKey
                    # Make a brand new subkey
                    newWxKey = ":".join([newWxCov, newType, newWxIntens]) + ":<NoVis>:"
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                            if newWxKey:  # Add the leading caret
                                newWxKey += "^"
                            newWxKey += (
                                ":".join([newWxCov, newType, newWxIntens]) + ":<NoVis>:"
                            )
                        else:
                            newWxKey += "^" + subKey
                ...
                replaceWxType = self.bothTypesObstructions(newWxType, wxKey)
                if replaceWxType:
                    newWxKey = self.replaceWxSubKey(
                        wxKey, replaceWxType, newWxCov, newWxType, newWxIntens
                    )
                else:  # just add it in to the existing key
                    newWxKey = self.makeNewWxKey(wxKey, newWxCov, newType, newWxIntens)
...
=======
            ...
            keyPartsList.append(keyPart)

        newKey = "^".join(keyPartsList)
        return newKey
    ...
    def popAssignsCBP(self, PoP, CloudBasePrimary, Sky, Visibility):
        changeMask = self.empty(np.bool)
        mask = (
            (PoP >= 15.0) & (PoP < 25.0) & (CloudBasePrimary > 110.0) & self._editAreaMask
        )
        CloudBasePrimary[mask] = 110.0  # 11000 feet
        changeMask |= mask
        mask = (PoP >= 25.0) & (PoP < 55.0) & (CloudBasePrimary > 100.0)
...
=======
    ...
    # Internal method to extract the Visibility limits based on wxType and coverage.
    def calcCoverageLimits(self, wxType, coverage):
        # need to copy so we don't mess with the original copy
        covList = copy.copy(self._wxTypeCovDict[wxType])
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
        covList.reverse()
        visList = [0.0]
...
=======
                ...
                # Calculate the FreezingFog mask
                freezingFogMask = (Visibility <= 0.625) & tBelow32
                if not freezingFogMask.any():  # skip if no point are found
                    continue
                ...
                # Update the potFreezingFogGrid grid
                potFreezingFogGrid[freezingFogMask] = potValue

            # Create the grids
            self.createGrid(self.mutableID(), "PotFog", "SCALAR", potFogGrid, tr)
            self.createGrid(
                self.mutableID(), "PotFreezingFog", "SCALAR", potFreezingFogGrid, tr
            )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                ...
                potValue = covDict[cov]
                potHazeGrid[mask] = potValue
                self.createGrid(self.mutableID(), "PotHaze", "SCALAR", potHazeGrid, tr)
        ...
        # Add Smoke
            ...
            for cov in covList:
                ...
                potValue = covDict[cov]
                potSmokeGrid[mask] = potValue
                self.createGrid(self.mutableID(), "PotSmoke", "SCALAR", potSmokeGrid, tr)
...
=======
        visFiveEightsToSix = (Visibility >= 0.625) & (Visibility <= 6.0)
        tBelow32 = T <= 32.0
        tAbove32 = T > 32.0

        noWxIndex = self.getIndex(self._noWxStr, wxKeys)

        # Define the criteria for Obstruction Wx based on the above masks
        # fogMask = ((T - Td) <= TMinusTd) & visFiveEightsToSix & self._editAreaMask
        fogMask = ((T - Td) <= TMinusTd) & self._editAreaMask
...
=======
                ...
                # Add Fog
                newWxGrid, newWxKeys = self.addNewWx(
                    wxGrid, wxKeys, cov, "F", noWxIntensity, mask
                )
                changeMask |= (mask & tAbove32)
                ...
                newWxGrid, newWxKeys = self.addNewWx(
                    ...
                    "ZF",
                    noWxIntensity,
                    mask & tBelow32 & visBelowFiveEighths,
                )
                changeMask |= (mask & tBelow32 & visBelowFiveEighths)
                ...
                newWxGrid, newWxKeys = self.addNewWx(
                    ...
                    "F",
                    "+",
                    mask & tAbove32 & visBelowPoint25,
                )
                changeMask |= (mask & tAbove32 & visBelowPoint25)
                ...
                newWxGrid, newWxKeys = self.addNewWx(
                    ...
                    "ZF",
                    "+",
                    mask & tBelow32 & visBelowPoint25,
                )
                changeMask |= (mask & tBelow32 & visBelowPoint25)
        ...
        # Add Haze to Wx
            ...
            for cov in covList:
                ...
                mask = (Visibility >= covMin) & (Visibility < covMax) & self._editAreaMask
                newWxGrid, newWxKeys = self.addNewWx(
                    wxGrid, wxKeys, cov, "H", noWxIntensity, mask
                )
                changeMask |= mask
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
        ...
        # Add Smoke to Wx
            ...
            for cov in covList:
                ...
                mask = (Visibility >= covMin) & (Visibility < covMax) & self._editAreaMask
                newWxGrid, newWxKeys = self.addNewWx(
                    wxGrid, wxKeys, cov, "K", noWxIntensity, mask
                )
                changeMask |= mask
        ...
        # Add Dust to Wx
            ...
            for cov in covList:
                ...
                mask = (Visibility >= covMin) & (Visibility < covMax) & self._editAreaMask
                newWxGrid, newWxKeys = self.addNewWx(
                    wxGrid, wxKeys, cov, "BD", noWxIntensity, mask
                )
                changeMask |= mask

        return (newWxGrid, newWxKeys), changeMask

    # Modifies the Visibility based on the Wx coverage and intensity.
    def updateVisFromWx(self, Wx, Visibility):
        changeMask = self.empty(np.bool)
        ...
        # Set Vis to 6.0 if FZFG
        wxMask = self.getWxMask(Wx, "ZF")
        visMask = (Visibility > 6.0) & self._editAreaMask
        Visibility[wxMask & visMask] = 6.0
        changeMask |= (wxMask & visMask)
        ...
        # Make masks and set the Vis to 6.0 for random wxTypes. Add more to this list
        # in order to have them considered.
        # make this a list for now in case new type need to be added later
        wxTypes = ["K"]
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            wxTypeList = (
                self._config["IntensPrecipList"] + self._config["IntensNonPrecipList"]
            )
        elif wxCat == "Non-Precip":
            wxTypeList = (
                self._config["IntensNonPrecipList"] + self._config["IntensPrecipList"]
            )
        else:
            print("ERROR! Invalid Intensity choice from GUI!!!!!!")
...
=======
                    ...
                    wxGrid, wxKeys = self.addNewWx(
                        wxGrid, wxKeys, subCov, subType, intenToAdd, mask
                    )
                    changeMask |= mask
...
=======
                newSubKey = (
                    ":".join([subParts[0], subParts[1], subParts[2], newVis]) + ":"
                )

            if len(subParts) == 5 and subParts[4]:
                newSubKey += subParts[4] + ":"
...
=======
        ...
        changeMask = np.ones(
            ceiling.shape, np.bool
        )  # Always reset all the Ceiling values
        ...
        for i in range(len(wxKeys)):
            ...
            for subKey in subKeys:
                ...
                if len(wxParts) >= 4:
                    ...
                    if visStr not in ["<NoVis>", ""]:
                        newSubKey = (
                            ":".join([wxParts[0], wxParts[1], wxParts[2], "<NoVis>"])
                            + ":"
                        )
...
=======
            visMask = (
                (Visibility >= visMin)
                & (Visibility < visMax)
                & (Visibility <= visThreshold)
                & self._editAreaMask
            )
            if not visMask.any():  # don't bother with empty masks
                continue
            ...
            for wxKey in wxKeys:
                ...
                if not overlap.any():  # don't bother with empty masks
                    continue
...
=======
            ...
            trList = self.getWEInventory(self.mutableID(), weName, timeRange)
            # Check for any gaps for this WE
            if not trList:
                # self.statusBarMsg("Some " + weName + " grids missing. Inventory check failed.", "S")
                return False
    ...
    # Returns true if the tool is configured to offer the Save and Publish option.
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
    ...
    def copyCeilingVisToCigHgtVsby(self):
        ...
        cigMin, cigMax = self.getParmMinMaxLimits(self.mutableID(), "CigHgt")

        for sourceWE, targetWE in weTuples:

            minValue, maxValue = self.getParmMinMaxLimits(self.mutableID(), targetWE)

            trList = self.getWEInventory(self.mutableID(), sourceWE)
            for tr in trList:
                sourceGrid = self.getGrids(self.mutableID(), sourceWE, "SFC", tr)

                # clip the grid in case the target has different min, max allowed values
                sourceGrid = sourceGrid.clip(minValue, maxValue)

                self.createGrid(self.mutableID(), targetWE, "SCALAR", sourceGrid, tr)
    ...
    def applyFinalizeRules(self, editArea):
        selectedTR = TimeRange.TimeRange(
            AbsTime.AbsTime(self._editStartTime), AbsTime.AbsTime(self._editEndTime)
        )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                self.statusBarMsg(
                    "Fcst gaps found but configured GapAction is set to Stop. Stopping Aviation_Finalize",
                    "S",
                )
                return
...
=======
            self.interpolateCmd(
                weList, selectedTR, "GAPS", "SYNC", interval=1, duration=1
            )
        ...
        # Remove Pot grids if configured
        if (
            self._config["MakePotFog"] == "Yes"
            and "Vis Adds New Wx" in self._userSelectedRules
        ):
            self.deleteCmd(
                ["PotFog", "PotFreezingFog", "PotHaze", "PotSmoke"], selectedTR
            )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
<<<<<<< HEAD
            ...
            if self.allDataAvailable(weDict, ["CloudBasePrimary", "Sky"], tr):
                weDict["Ceiling"][tr] = self.makeCeiling(weDict["CloudBasePrimary"][tr][0],
                                                         weDict["Sky"][tr][0])                   
=======
                    weDict["CloudBasePrimary"][tr] = self.popAssignsCBP(
                        weDict["PoP"][tr][0],
                        weDict["CloudBasePrimary"][tr][0],
                        weDict["Sky"][tr][0],
                        weDict["Visibility"][tr][0],
                    )
            ...
            if "PoP Defines Sky" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["PoP", "Sky"], tr):
                    weDict["Sky"][tr] = self.popAssignsSky(
                        weDict["PoP"][tr][0], weDict["Sky"][tr][0]
                    )

            if "Vis Adds New Wx" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["Visibility", "T", "Td", "Wx"], tr):
                    grid, mask = self.addNewWxFromVis(
                        weDict["Visibility"][tr][0],
                        weDict["T"][tr][0],
                        weDict["Td"][tr][0],
                        weDict["Wx"][tr][0],
            ...
            if "Wx Adjusts Vis" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["Wx", "Visibility"], tr):
                    grid, mask = self.updateVisFromWx(
                        weDict["Wx"][tr][0], weDict["Visibility"][tr][0]
                    )
                    finalMask = (
                        weDict["Visibility"][tr][1] | mask
                    )  # Accumulate the points
                    weDict["Visibility"][tr] = grid, finalMask

            if "QC Vis with PoP & Wx" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["PoP", "Wx", "Visibility"], tr):
                    grid, mask = self.lowPoPNoWxVis10(
                        weDict["PoP"][tr][0],
                        weDict["Wx"][tr][0],
                        weDict["Visibility"][tr][0],
                    )
            ...
            if "Update Wx Intensity Using Vis" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["Visibility", "Wx"], tr):
                    grid, mask = self.updateWxIntensWithVis(
                        weDict["Visibility"][tr][0], weDict["Wx"][tr][0]
                    )
                    finalMask = weDict["Wx"][tr][1] | mask  # Accumulate the points
                    weDict["Wx"][tr] = grid, finalMask
            ...
            if "Adjust WindGust Using Wind" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["Wind", "WindGust"], tr):
                    weDict["WindGust"][tr] = self.updateWindGustWithWind(
                        weDict["Wind"][tr][0], weDict["WindGust"][tr][0]
                    )

            if "Add Vis to Wx String" in self._userSelectedRules:
                if self.allDataAvailable(weDict, ["Wx", "Visibility"], tr):
                    grid, mask = self.setWxVis(
                        weDict["Wx"][tr][0],
                        weDict["Visibility"][tr][0],
                        self._addVisBelow,
                    )
            ...
            if self.allDataAvailable(
                weDict, ["CloudBasePrimary", "Sky", "Visibility"], tr
            ):
                grid, mask = self.noZeroCeiling(
                    weDict["CloudBasePrimary"][tr][0],
                    weDict["Sky"][tr][0],
                    weDict["Visibility"][tr][0],
                )
                finalMask = (
                    weDict["CloudBasePrimary"][tr][1] | mask
                )  # Accumulate the points
                weDict["CloudBasePrimary"][tr] = grid, finalMask

            if self.allDataAvailable(weDict, ["CloudBasePrimary", "Sky"], tr):
                weDict["Ceiling"][tr] = self.makeCeiling(
                    weDict["CloudBasePrimary"][tr][0], weDict["Sky"][tr][0]
                )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                ...
                if tr in weDict[weName]:
                    ...
                    if not mask.any():  # don't modify grids that don't need it
                        continue
                    else:  # save the grid
                        self.createGrid(self.mutableID(), weName, weType, grid, tr)
...
=======
        ...
        # Run button
        self._runButton = tk.Button(
            self._bottomButtonFrame, text="Run", command=self.runCommand
        )
        self._runButton.grid(row=0, column=0, padx=20)

        # Run/Dismiss buttom
        self._runDismissButton = tk.Button(
            self._bottomButtonFrame, text="Run/Dismiss", command=self.runDismissCommand
        )
        self._runDismissButton.grid(row=0, column=1, padx=20)
        # Cancel button
...
=======
    ...
    def ruleSelected(self, event, ruleName):
        ...
        # toggle on
        if (
            ruleName not in self._userSelectedRules
            and self._ruleDict[ruleName].cget("state") == tk.ACTIVE
        ):
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            ...
            ruleName in self._userSelectedRules
            and self._ruleDict[ruleName].cget("state") == tk.ACTIVE
        ):
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            ...
            button_ttp = CreateToolTip(
                self._ruleDict[ruleName], self._config["toolTips"][ruleName]
            )
...
=======
        for cov in covList:
            # Update the label
            label = (
                cov + " " + wxType + " where Vis < " + "%.2f" % self._scales[cov].get()
            )
            if cov == covList[0]:  # last item
                label = (
                    cov
...
=======
            # scale = self._scales[self._TMinusTdScaleName]
            label = "Fog where T - Td <= " + str(
                self._scales[self._TMinusTdScaleName].get()
            )
            self._scales[self._TMinusTdScaleName].config(label=label)
            self._scales[self._TMinusTdScaleName].grid_propagate(0)
...
=======
    ...
    def makeCovSliders(self, wxType):
        ...
        self._sliderFrame = tk.Frame(
            self._addWxFrame, relief=tk.GROOVE, bd=3, bg=self._bgColor
        )
        self._sliderFrame.grid(
            row=self._startCovRow, column=0, sticky=tk.N + tk.E + tk.S + tk.W
        )
...
=======
            self._scales[self._TMinusTdScaleName] = tk.Scale(
                self._sliderFrame,
                from_=0,
                to=10,
                orient=tk.HORIZONTAL,
            ...
            self._scales[self._TMinusTdScaleName].grid(
...
=======
        ...
        self._addWxFrame = tk.Frame(frame, relief=tk.GROOVE, bd=3, bg=self._bgColor)
        self._addWxFrame.grid(
            row=0, column=1, sticky=tk.N + tk.E + tk.S + tk.W, columnspan=2
        )
...
=======
        wxTypeList = sorted(self._wxTypeCovDict)
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            name = wxTypeList[i]
            self._wxTypeDict[name] = tk.Radiobutton(
                self._addWxFrame,
                text=wxTypeList[i],
                padx=10,
                pady=2,
                command=self.wxTypeSelected,
                variable=self._selectedWxType,
                value=wxTypeList[i],
            )
            self._wxTypeDict[name].grid(row=row, sticky=tk.W)
...
=======
    ...
    def displayAddVisToWxFrame(self):
        self._addVisToWxFrame.grid(
            row=self._addVisToWxRow, column=0, sticky=tk.N + tk.E + tk.S + tk.W
        )
        return
    ...
    def displayIntensityFrame(self):
        self._intFrame.grid(
            row=self._addVisToWxRow, column=1, sticky=tk.N + tk.E + tk.S + tk.W
        )
        return
...
=======
    ...
    def makeAddVisToWxScale(self, frame, row):
        ...
        self._addVisToWxFrame = tk.Frame(frame, relief=tk.GROOVE, bd=3)
        self._addVisToWxFrame.grid(
            row=self._addVisToWxRow, column=0, sticky=tk.N + tk.S + tk.E + tk.W
        )
        ...
        self._addVisToWx = tk.DoubleVar()
        self._addVisToWx.set(self._config["AddVisToWxValue"])
        self._addVisToWxScale = tk.Scale(
            self._addVisToWxFrame,
            from_=0,
            to=10,
            orient=tk.HORIZONTAL,
            command=self.addVisToWxSliderChanged,
...
=======
        for choice in choiceList:
            button = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.intensitySelected,
...
=======
        ...
        savePubFrame = tk.Frame(frame, relief=tk.GROOVE, bd=3)
        savePubFrame.grid(
            row=row, column=2, sticky=tk.N + tk.S + tk.E + tk.W, columnspan=3
        )
...
=======
        for choice in choiceList:
            button = tk.Radiobutton(
                ...
                padx=10,
                pady=2,
                command=self.savePubSelected,
                variable=self._savePublishVar,
                value=choice,
            )
            button.grid(row=row)
            row += 1
...
=======
        ...
        startHour = self._config["TAFStartHour"]
        endHour = self._config["TAFEndHour"]
        self._startTime = int(self._gmtime().unixTime() / 3600) * 3600 + (
            startHour * 3600
        )
        self._endTime = self._startTime + (endHour * 3600)
...
=======
                if (
                    self._editEndTime > self._endTime
                ):  # reset the display end time so everything fits
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11

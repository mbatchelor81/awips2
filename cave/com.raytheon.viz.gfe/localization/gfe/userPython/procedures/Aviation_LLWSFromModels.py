...
=======
# Aviation_LLWSFromModels - Version 20191212
#
# Author:  Matthew H. Belk                  Created: 10/29/2009
#          WFO Norton MA              Last Modified: 12/12/2019
# ----------------------------------------------------------------------------
# SOFTWARE HISTORY
...
# Modified to work in both A1 and A2 environments by Tom LeFebvre - 6/20/14
# Renamed to Aviation_ and removed A1 any code. - 2/29/16 - Tom LeFebvre
# Fixed a few model inventory issues - 03/03/17 - Tom LeFebvre
# Modified to more efficient numpy syntax - 04/13/17 - Matthew Belk (BOX)
# Removed references to smart tools which are no longer needed - Tom LeFebvre 2/23/18
# Refactored code into common utility - Tom LeFebvre 11/14/18
# Dec 17, 2021 8342       sharbison    Changes for Performance Logging.
# Dec 13, 2022 8986       sharbison    Bug fix for Performance Logging.
# ===============================================================================
...
# Define a toggle to create some debug grids - can also be used for
# training purposes
CreateDebugGrids = False  # True = Yes, False = No
CreateVectorComponents = False  # True = Yes, False = No
...
for model in ModelList:
    ...
    # Add a slider bar to handle the last two versions this model
    VariableList.append((model, 0, "scale", [0, 10], 1))
    VariableList.append((f"Previous {model}", 0, "scale", [0, 10], 1))

    # Add these names to the dialog list so we can get their data later on
    dialogNames.append(model)
    dialogNames.append(f"Previous {model}")


class Procedure(
    ModelBlendUtility.ModelBlendUtility,
    GridManipulation.GridManipulation,
    SmartScript.SmartScript,
):
    def __init__(self, dbss):
    ...
    def _processVariableList(self, varDict):
        ...
        # Get name of chosen model(s) - and fix it up so we can use it later
        # on.  This will grab the latest version of the chosen model.
        self._models, self._modelWeights = self._aviationUtils.getModelWeights(
            dialogNames, varDict
        )
    ...
    def _computeShear(self, uBottom, vBottom, uTop, vTop, depth):
        ...
        duSquared = (uTop - uBottom) ** 2
        dvSquared = (vTop - vBottom) ** 2
...
<<<<<<< HEAD
                ...
                if tr not in modelInventory:
                    ...
                    wind2000 = self.GM_interpolateGrid(model, "wind", "BL3060",
                                                       tr, modelInventory)
=======
                # Fetch the grids we need for this time step
                if tr not in modelInventory:
                    wind1000 = self.GM_interpolateGrid(
                        model, "wind", "BL030", tr, modelInventory
                    )
                    wind2000 = self.GM_interpolateGrid(
                        model, "wind", "BL3060", tr, modelInventory
                    )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                ...
                if wind1000 is None or wind2000 is None:
                    self.statusBarMsg(model + " BL winds are missing at: " + str(tr), "A")
                    continue
                # ---------------------------------------------------------------
                # Convert these winds into U and V vector components
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                ...
                u1000 *= float(self._modelWeights[model])
                v1000 *= float(self._modelWeights[model])
                u2000 *= float(self._modelWeights[model])
                v2000 *= float(self._modelWeights[model])
                ...
                count += float(self._modelWeights[model])

            perfLog.logDuration("Fetching Aviation_LLWS model grids", time.perf_counter() - t1)
...
=======
            ...
            if Wind is None:
                self.statusBarMsg(f"Surface Wind grids are missing at: {tr}", "S")
            ...
            if Wind is not None:
                (uSfc, vSfc) = self.MagDirToUV(
                    Wind[0] / self.convertMsecToKts(1), Wind[1]
                )
            ...
            if CreateVectorComponents:
                ...
                # Display U and V components of all the winds we have so far
                self.createGrid(
                    "Fcst", "uSfc", "SCALAR", uSfc.astype(float32), tr, precision=2
                )
                self.createGrid(
                    "Fcst", "vSfc", "SCALAR", vSfc.astype(float32), tr, precision=2
                )
                ...
                self.createGrid("Fcst", "count", "SCALAR", self.newGrid(count), tr)

            # -------------------------------------------------------------------
            # Compute a mean wind around 500 ft - if we can

            if (
                avgU1000 is not None
                and avgV1000 is not None
                and uSfc is not None
                and vSfc is not None
            ):
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
                ...
                if CreateVectorComponents:
                    ...
                    # Display U and V components of average BL wind
                    self.createGrid(
                        "Fcst", "u500", "SCALAR", u500.astype(float32), tr, precision=2
                    )
                    self.createGrid(
                        "Fcst", "v500", "SCALAR", v500.astype(float32), tr, precision=2
                    )
                    self.createGrid(
                        "Fcst",
                        "wind500",
...
=======
            ...
            if (
                avgU1000 is not None
                and avgV1000 is not None
                and avgU2000 is not None
                and avgV2000 is not None
            ):
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
            ...
            if (
                uSfc is not None
                and vSfc is not None
                and u500 is not None
                and v500 is not None
            ):

                shear = self._computeShear(uSfc, vSfc, u500, v500, self.convertFtToM(500))
                ...
                if CreateDebugGrids:
                    ...
                    self.createGrid(
                        ...
                        "shear0005",
                        "SCALAR",
                        shear.astype(float32),
                        tr,
                        precision=3,
                        minAllowedValue=0.0,
                        maxAllowedValue=0.25,
                        units="/s",
                        defaultColorTable="LLWS_500",
                    )
                ...
                # Determine difference of shear and the LLWS threshold
                diffShear = np.where(
                    shear >= shearThreshold, (shear - shearThreshold), -1
                )

                # Define a mask where shear is maximum shear computed so far
                maxShearMask = diffShear >= maxDiffShear

                # Define a mask where shear meets LLWS criterion for this layer
                # depth and this is the maximum shear computed so far
                llwsMask = (shear >= shearThreshold) & maxShearMask

                # Update the max shear and height grids
                maxShear = np.maximum(shear, maxShear)
                maxDiffShear[maxShearMask] = diffShear[maxShearMask]
                maxHeight[llwsMask] = 5
            ...
            if (
                ...
                shear = self._computeShear(
                    u500, v500, avgU1000, avgV1000, self.convertFtToM(500)
                )
                ...
                # Update the max shear and height grids
                mask = shear > maxShear
                maxShear[mask] = shear[mask]
                maxDiffShear[maxShearMask] = diffShear[maxShearMask]
                maxHeight[llwsMask] = 10

                # Update wind magnitude and direction to report in LLWS group
                maxMag[llwsMask] = wind1000[0][llwsMask]
                maxDir[llwsMask] = wind1000[1][llwsMask]
            ...
            if (
                avgU1000 is not None
                and avgV1000 is not None
                and u1500 is not None
                and v1500 is not None
            ):

                shear = self._computeShear(
                    avgU1000, avgV1000, u1500, v1500, self.convertFtToM(500)
                )
                ...
                maxShear[mask] = shear[mask]
                maxDiffShear[maxShearMask] = diffShear[maxShearMask]
                maxHeight[llwsMask] = 15

                # Update wind magnitude and direction to report in LLWS group
                maxMag[llwsMask] = wind1500[0][llwsMask]
                maxDir[llwsMask] = wind1500[1][llwsMask]
            ...
            if (
                ...
                shear = self._computeShear(
                    u1500, v1500, avgU2000, avgV2000, self.convertFtToM(500)
                )
                ...
                maxShear[mask] = shear[mask]
                maxDiffShear[maxShearMask] = diffShear[maxShearMask]
                maxHeight[llwsMask] = 20

                # Update wind magnitude and direction to report in LLWS group
                maxMag[llwsMask] = wind2000[0][llwsMask]
                maxDir[llwsMask] = wind2000[1][llwsMask]
            ...
            if (
                uSfc is not None
                and vSfc is not None
                and avgU1000 is not None
                and avgV1000 is not None
            ):

                shear = self._computeShear(
                    uSfc, vSfc, avgU1000, avgV1000, self.convertFtToM(1000)
                )
            ...
            if (
                ...
                shear = self._computeShear(
                    avgU1000, avgV1000, avgU2000, avgV2000, self.convertFtToM(1000)
                )
            ...
            if (
                ...
                shear = self._computeShear(
                    uSfc, vSfc, u1500, v1500, self.convertFtToM(1500)
                )
            ...
            if (
                ...
                shear = self._computeShear(
                    u500, v500, avgU2000, avgV2000, self.convertFtToM(1500)
                )
            ...
            if (
                ...
                shear = self._computeShear(
                    uSfc, vSfc, avgU2000, avgV2000, self.convertFtToM(2000)
                )
            ...
            perfLog.logDuration("Calculating LLWS and LLWSHgt grids", time.perf_counter() - t2)

        self.statusBarMsg("Aviation_LLWS completed...", "R")
        perfLog.logDuration("Calculating Aviation_LLWS grid models", time.perf_counter() - t0)
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11

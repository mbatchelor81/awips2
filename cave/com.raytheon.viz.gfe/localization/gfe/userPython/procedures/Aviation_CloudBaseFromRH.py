...
=======
# Aviation_CloudBaseFromRH  - Version 20191212
#
# Author:  Matthew H. Belk                  Created: 01/22/2010
...
VariableList.append(
    (
        ...
        "CloudBasePrimary",
        "radio",
        ["CloudBasePrimary", "CloudBaseSecondary", "CloudBaseConditional"],
    )
)
...
VariableList.append(
    (
        "Adjusting the slider bar below to the\n"
        + "right will make it tougher to detect a\n"
        + "cloud, which should raise cloud heights.\n"
        + "Moving the slider bar to the left,\n"
        + "will make it easier to detect a cloud.",
        "",
        "label",
    )
)
...
# Add a control to alter RH thresholds
VariableList.append(("Alter RH thresholds for clouds by (%):", 0, "scale", [-20, 20], 1))
...
=======
        ...
        # function. These must be in ascending order to satisfy scipy.
        # Physically, this is opposite order we need.
        for key in sorted(self._cloudRH, reverse=True):
            rhKnots.append(self.newGrid(self._cloudRH[key] + alterRHthresholds))
            # presCube.append(self.newGrid(key))
        ...
        # in the GFE-proper order later on.
        empty = self.empty()
        self.BI_col, self.BI_row = np.meshgrid(
            np.arange(empty.shape[1]), np.arange(empty.shape[0])
        )
    ...
    def execute(self, editArea, timeRange, varDict):
        ...
        if CreateDebugGrids:
            self.createGrid(
                ...
                topo,
                timeRange,
                minAllowedValue=-3000.0,
                maxAllowedValue=25000.0,
            )
...
=======
                self.statusBarMsg(
                    f"{self._populateHeight} grid is locked at: {tr}", "S"
                )
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
                continue
...
=======
            ...
            for model in self._models:
                ...
                modelInventory = self.GM_getWEInventory("rh", model, self._levels[0])

                # Fetch the cubes
                if tr not in modelInventory:
                    sounding = self.GM_interpolateSounding(
                        model, "rh", self._levels, tr, modelInventory
                    )
                else:
                    sounding = self.makeNumericSounding(
                        model, "rh", self._levels, tr, noDataError=0
                    )

                if sounding is None:
                    self.statusBarMsg(model + " sounding is missing at: " + str(tr), "A")
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
                    continue
...
=======
            ...
            cloudBaseHgt = cloudBaseHgt.clip(1, 250)
            self.createGrid("Fcst", self._populateHeight, "SCALAR", cloudBaseHgt, tr)

        perfLog.logDuration("Calculating aviation cloud base from RH", time.perf_counter() - t0)

...
=======
# Aviation_EDASConfig - Version 20191212
#
...
=======
config["Populate"]["genElementList"] = [
    "CloudBasePrimary",
    "Visibility",
    "LLWS",
    "LLWSHgt",
...
# The set of generic elements that default to "on" when the GUI first appears
config["Populate"]["genElementDefaultList"] = [
    "CloudBasePrimary",
    "Visibility",
    "LLWS",
    "LLWSHgt",
...
=======
config["Populate"]["conElementList"] = [
    "CloudBasePrimary",
    "CloudBaseSecondary",
    "CloudBaseConditional",
]
# The element selected when the GUI first appears.
config["Populate"]["conElementDefault"] = "CloudBaseSecondary"
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
config["Timer"]["valueDict"] = {
    "Visibility": (0.0, 10.0, 2.0),
    "Sky": (0.0, 100.0, 20.0),
    "SkyPrimary": (0.0, 100.0, 20.0),
    "SkySecondary": (0.0, 100.0, 20.0),
    "CloudBasePrimary": (0.0, 250.0, 50.0),
    "CloudBaseSecondary": (0.0, 250.0, 50.0),
    "VisibilityConditional": (0.0, 10.0, 2.0),
    "CloudBaseConditional": (0.0, 250.0, 50.0),
}
...
config["Timer"]["colors"] = {
    ...
    "SkySecondary": "blue",
    "CloudBasePrimary": "red",
    "CloudBaseSecondary": "darkgreen",
    "VisibilityConditional": "orange",
    "CloudBaseConditional": "darkred",
}
...
=======
# config["Timer"]["editAreaList"] = ["AddYour", "EditArea", "ListHere"]
config["Timer"]["editAreaList"] = ["KDENDAS", "KBJCDAS", "KAPADAS"]

# Set defaultEditAreas to the edit areas (above) that you wish to be selected when the tool starts.
# This list can contain only the edit areas defined in editAreaList.
config["Timer"]["defaultEditAreas"] = ["KDENDAS"]

config["Timer"]["reportableValues"] = {}
visReportableValues = [
    (0.0, 0.375, 0.0625),
    (0.375, 1.875, 0.125),
    (1.875, 3.0, 0.25),
...
config["Timer"]["reportableValues"]["Visibility"] = visReportableValues
config["Timer"]["reportableValues"]["VisibilityConditional"] = visReportableValues

skyReportableValues = [(0.0, 100.0, 5.0)]
config["Timer"]["reportableValues"]["Sky"] = skyReportableValues
config["Timer"]["reportableValues"]["SkyPrimary"] = skyReportableValues
config["Timer"]["reportableValues"]["SkySecondary"] = skyReportableValues

cloudBaseReportableValues = [
    (0.0, 50.0, 1.0),
    (50.0, 100.0, 5.0),
    (100.0, 300.0, 10.0),
]
config["Timer"]["reportableValues"]["CloudBasePrimary"] = cloudBaseReportableValues
config["Timer"]["reportableValues"]["CloudBaseSecondary"] = cloudBaseReportableValues
config["Timer"]["reportableValues"]["CloudBaseConditional"] = cloudBaseReportableValues
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
...
=======
config["Timer"]["zoomFactor"]["CloudBaseConditional"] = 3.0
config["Timer"]["zoomFactor"]["VisibilityConditional"] = 2.0

# Worsening only item. If set to "Yes", only worsening conditions will be allowed
# If set to "no" (or anything but "Yes") any edits better or worse will be allowed
config["Timer"]["WorseningOnly"] = "Yes"
...
=======
config["Finalize"]["AllChecks"] = [
    "PoP Adjusts Cloud Base",
    "QC Vis with PoP & Wx",
    "PoP Defines Sky",
    "Vis Adds New Wx",
    "Wx Adjusts Vis",
...
# The subset of checks above that will be defaulted to on when the tool is started.
config["Finalize"]["DefaultOnChecks"] = [
    "QC Vis with PoP & Wx",
    "Adjust WindGust Using Wind",
]
...
# The set of rules where no selection is offered. Adding rules to this list makes them
# unchangeable by any user in the GUI whether the they default to on or off.
config["Finalize"]["ImmutableChecks"] = [
    "Adjust WindGust Using Wind",
    "Add Vis to Wx String",
]
...
# Define the text to be displayed for tool tips
config["Finalize"]["toolTips"] = {}

config["Finalize"]["toolTips"]["PoP Adjusts Cloud Base"] = (
    "Where 15% <= PoP < 25% and CBP > 11000 ft.: \n ...CloudBasePrimary = 11000 ft. \n"
    + "Where 25% <= PoP < 55% and CBP > 10000 ft.: \n ...CloudBasePrimary = 10000 ft. \n"
    + "Where PoP >= 55% and CBP > 9000 ft.:\n ...CBP = 9000 ft."
)
# config["Finalize"]['toolTips']["QC Vis with PoP & Wx"] = "Where PoP < 55% and No Wx:\nVisibility = 7"
config["Finalize"]["toolTips"][
    "QC Vis with PoP & Wx"
] = "If  Pop < 55 and No Wx/Obstructions:\nVis = 7 miles"
config["Finalize"]["toolTips"]["PoP Defines Sky"] = "Where PoP > Sky:\nSky = PoP"
config["Finalize"]["toolTips"]["Vis Adds New Wx"] = (
    "Where Visibility < Coverage Value: Add-> \n-- Fog where T - Td < Threshold and No Wx\n"
    + "-- Haze where No Wx and No Fog\n-- Smoke where No Wx"
)

config["Finalize"]["toolTips"]["Wx Adjusts Vis"] = (
    "Where No Wx and Vis < 7 miles: Vis = 7 miles\n"
    + "Where Fog and Vis > 6 miles: Vis = 6 miles\n"
    + "Where Precip. and Vis > 6 miles: Vis = 6 miles\n"
    + "Where Heavy Fog (F+) and Vis > 0.25 miles: Vis = 0.25 miles\n"
    + "Where Moderate Snow (Sm) and Vis > 0.5 miles: Vis = 0.5 miles\n"
...
config["Finalize"]["toolTips"]["Update Wx Intensity Using Vis"] = (
    "For S, SW, L, ZL, F, ZF Wx Intensity...\n"
    + "is Heavy (+) where Vis <= 0.25 miles\n"
    + "is Moderate (m) where Vis <= 0.5 miles\n"
    + "is Light (-) where Vis > 0.5 miles"
)
config["Finalize"]["toolTips"]["Adjust WindGust Using Wind"] = (
    "Where WindSpeed >= WindGust:\n" + "WindGust = WindSpeed"
)
config["Finalize"]["toolTips"][
    "Add Vis to Wx String"
] = "Add the Visibility Value to the Wx Key"
...
=======
config["Finalize"]["IntensPrecipList"] = [
    "L",
    "S",
    "SW",
]

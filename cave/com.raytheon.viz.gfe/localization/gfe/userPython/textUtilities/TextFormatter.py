...
# ------------- -------- --------- --------------------------------------------
# Feb 12, 2014  2591     randerso  Added retry when loading combinations fails
# Oct 20, 2014  3685     randerso  Changed default of lowerCase to True if not
#                                  specified
# Nov 30, 2015  5129     dgilling  Support new IFPClient.
# Mar 02, 2016  5411     randerso  Fixed exception in exception handler
...
from org.locationtech.jts.geom import GeometryFactory, LinearRing, Coordinate, Polygon
        ...
        try:
            ...
            # Smart Text
                ...
                if argDict.get('testMode', 0):
                    ...
                    #split by "$$"
                    segs = text.split('\n$$')
                    for i in range(len(segs) - 1):  #never the last one
            ...
            deleteAllTempReferenceData(self.dataMgr.getRefManager())
...
#        endText = self.__ut.set(forecastDef, "endingText", "")
                    ...
                    if citydict is None:
                        ...
                        for ea in editAreas:
                            for ean in ea:
                                ...
                                for city, llrec in citydict[ean].items():
                                    # Create a referenceData given lat, lon, dim
    ...
    def getFcstDef(self, fcstName, argDict):
        ...
        try:
            ...
            method = getattr(product, fcstName)
            fcstDef = method()
            ...
            if fcstDef is None or type(fcstDef) is not dict:
                ...
                try:
                    # Look for Definition = {}
                    #  This can be removed eventually
                    fcstDef = module.Definition
                except:
                    try:
                        # Look for fcstName = {}
                        # This can be removed eventually
                        fcstDef = getattr(module, fcstName)
                    except:
                        try:
    ...
    def getEditArea(self, editAreaName, argDict):
        ...
        if "editAreaSuffix" in definition:
...
def makeSquare(lat, lon, km):
    ...
    points = []
    points.append(repr(latTop) + "," + repr(lonRight))
    points.append(repr(latTop) + "," + repr(lonLeft))
    points.append(repr(latBottom) + "," + repr(lonLeft))
    points.append(repr(latBottom) + "," + repr(lonRight))
    return points
...
# Remove all temporary reference data from the server
def deleteAllTempReferenceData(refSetMgr):
    global LatLonIds
    for refId in LatLonIds:
        refSetMgr.deleteRefSet(refId)
    LatLonIds = []


def getTime():
    "Return an ascii string for the current time without spaces or :'s"
    timeStr = repr(time.time())
...
def writeToServerFile(forecasts, outputFile, ifpClient):
    if not outputFile is None and outputFile != "":
        textFileId = AFPS.TextFileID(outputFile, "PRODGEN")
        textFile = AFPS.TextFile(textFileId, forecasts)

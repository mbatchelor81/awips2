...
#        TimeRangeUtils.TimeRangeUtils.__init__(self)
    ...
    def makeAreaHeader(self, argDict, areaLabel, issueTime, expireTime,
        ...
        # Determine list of areas (there could be more than one if we are using
        # a combination)
        ...
        if accurateCities and \
           len(ugcCityList) > 0 and "hazards" in argDict:
        ...
        VTECString = ""
        VTECRecords = []
        if "hazards" in argDict and includeVTECString:
        ...
        if type(expireTime) in [int, float]:
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
        ...
        nameString = self.endline(
            nameString, linelength=self._lineLength, breakStr=["-"])
        if cityString != "":
            ...
            if numCities == 1:
                def preserveCase(matchobj):
                    ...
                    for i in range(len(repl)):
                        ...
                        retv = retv + c
                    return retv
                cityDescriptor = re.sub(
                    "cities", preserveCase, cityDescriptor, flags=re.IGNORECASE)
        ...
        if includeVTECString == 0:
            VTECString = ""
        header = codeString + VTECString + \
            nameString + cityString + issueTimeStr
        return header
    ...
    def makeAccurateCityList(self, areaList, ugcCityList, argDict):
        ...
        cityString = self.punctuateList(citylist)
    ...
    def makeUGCList(self, areaDict, areaList):
        ...
        areaUgcList.sort(key=self.ugcSort)
    ...
    def expandComplexUgc(self, complexUgc):
        ...
        while len(complexUgc) > 0:
            ...
            nextUgcs, curState, lastNum = self.expandUgc(
                ugc, curState, lastNum, arrowFlag)
            arrowFlag = 0
            if sep == arrow:
                arrowFlag = 1
    ...
    def getGeneralAreaList(self, areaList, areaDictName="AreaDictionary"):
        ...
        gaLAND = []
        gaWATER = []
        for ((g, pg), names) in geoAreas.items():
            words = g.split(' ')
            found = 0
            for w in waters:
                if w in words:
    ...
    def formatCountyString(self, state, counties):
        ...
        return result + self.punctuateList(counties)
    ...
    def getIssuedByString(self, words="Issued by National Weather Service "):
        ...
        try:
            ...
            if len(issuedByString) > self._lineLength:
                issuedByString = issuedByString.replace("National Weather Service", "NWS")
...
#############################
    ...
    def getAreaDictEntry(self, accessArg, areaDict, entryName, firstOnly=0):
        ...
        entryValues = []
        for areaLabel in areaDict:
            if accessArg != "all":
                if type(accessArg) is dict:
                    # tree or argDict
                    if not self.currentAreaContains(tree, [areaLabel]):
                        continue
                if type(accessArg) is list:
                    # list of areas
                    if areaLabel not in accessArg:
                        continue
            entry = areaDict[areaLabel]

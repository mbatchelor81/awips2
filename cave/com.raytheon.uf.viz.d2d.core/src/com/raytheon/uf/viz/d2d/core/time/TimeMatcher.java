...
public class TimeMatcher {
    ...
    private static final long ONE_MINUTE_MS = TimeUtil.MILLIS_PER_MINUTE;
    ...
    // 1 day in seconds
    private static final long ONE_DAY_S = 86_400;
    ...
    // 100 days in seconds
    private static final long ONE_HUNDRED_DAYS_S = 100 * ONE_DAY_S;

    public static final float DEFAULT_TOLERANCE_FACTOR = 0.6f;

    private static final long[] autoIntervals = { 300, 900, 1800, 3600, 10_800,
            21_600, 43_200, ONE_DAY_S };
    ...
    // ---------------------------------------------------------------------------
    private static int filterOldForecasts(List<DataTime> times, Date latest,
            boolean noBackfill) {
        int next, check;
        int lastRun = -1;
        long curRef;
    ...
    // ---------------------------------------------------------------------------
    private static void validTimeSort(List<DataTime> times,
        ...
        Collections.sort(times,
                new DataTimeComparator(DataTimeComparator.SortKey.VALID_TIME,
        ...
        if (times.get(i).isSpatial() && !ignoreSpatial) {
            ...
            for (j = i + 1; j < times.size(); i++, j++) {
                if (times.get(j).getMatchValid() > times.get(i)
                        .getMatchValid()) {
    ...
    // ---------------------------------------------------------------------------
    private IntrinsicReturnVal intrinsicPeriod(DataTime[] times,
    ...
    // ---------------------------------------------------------------------------
    private IntrinsicReturnVal intrinsicPeriod(List<DataTime> times,
        ...
        d = ((times).get((majorIndex).get(m)).getMatchValid()
                - (times).get((majorIndex).get(0)).getMatchValid()) / nn;
        if (d < 0) {
            d = -d;
        }
        if (d >= dt && d < dt * 1.05) {
    ...
    private static void makeFinalLoadList(List<DataTime> loadTimes, int nFrames,
        ...
        if (prefered.getTime() > 0) {
            long dt = 0x7FFFFFFF;
            boolean byRef = (loadTimes).get(prefIdx)
                    .getMatchValid() == (loadTimes).get(0).getMatchValid();
            long thisTime;
    ...
    public DataTime[] doValTimOverlay(DataTime[] depictTimeArr,
        ...
        if (mode == LoadMode.RANGE_MATCH) {
            ...
            for (DataTime depictTime : depictTimeArr) {
                if (depictTime != null) {
                    if (depictTime.getValidPeriod() == null || depictTime
                            .getValidPeriod().getDuration() < (1000 * 60)) {
        ...
        if (latest != null) {
            if (mode == LoadMode.NO_BACKFILL) {
                filterOldForecasts(depictTimes, latest, true);
                    ...
                    || mode == LoadMode.PREVIOUS_MODEL_RUN_1
                    || mode == LoadMode.PREVIOUS_MODEL_RUN_2) {
    ...
    // ---------------------------------------------------------------------------
    private static List<DataTime> filterByForecast(DataTime[] incomingTimes,
            long forecast) {
        int next, check;
        ...
        if (forecast >= ONE_HUNDRED_DAYS_S || forecast < 0) {
            if (p == 0) {
                return times;
            }
            if (forecast > 0) {
    ...
    private static Date filterByClock(List<DataTime> times, Date clock,
        ...
        long latest = 0L;
        long curRef;
        if (times == null || times.isEmpty() || clock == null) {
            return clock;
        }
    ...
    // ---------------------------------------------------------------------------
    private static Date makeRowloadList(List<DataTime> depictTimes, Date latest,
    ...
    private static void filterByDeltaTime(List<DataTime> times, long deltaTime,
            Date preferred) {
        ...
        Calendar preferredCal = Calendar
        ...
        long allowableDelta = Math.min(deltaTime / 2, 900_000);
        ...
        times.clear();
        for (Iterator<Long> keyIter = expectedTimesMap.keySet()
    ...
    // data for each available valid time. Returns valid time of preferred
    // frame.
    // Caller creates and owns both depictTimes and loadTimes.
    // loadTimes is assumed to be an empty sequence when call is made.
    // ---------------------------------------------------------------------------
    private static Date makeValTimSeqList(List<DataTime> depictTimes,
            Date latest, List<DataTime> loadTimes) {
        int p, q;
    ...
    private static Date makeDprogDtList(List<DataTime> depictTimes,
        ...
        // base valid time of the dProg/dt
        v = (depictTimes).get(p).getMatchValid();
    ...
    // Given a sequence of available DataTimes for an able to depict, the clock
    // time,
    ...
    public static DataTime[] makeEmptyLoadList(DataTime[] depictTimes,
            ...
            long deltaTime) {
        List<DataTime> loadTimes = new ArrayList<>();
        ...
        if (mode == LoadMode.STD) {
            ...
            filterByDeltaTime(loadTimes, deltaTime, preferred);
            makeFinalLoadList(loadTimes, nFrames, preferred, preferredLast);
        } else if (mode == LoadMode.LATEST) {
            List<DataTime> filteredTimes = filterByForecast(depictTimes,
                    filterTime);
            if (filteredTimes.isEmpty()) {
            ...
            // for previous - 1 and -2 , repeat filterByClock call
            if (mode == LoadMode.PREVIOUS_MODEL_RUN_1) {
                for (int i = 0; i < 1; i++) {
                    latest = filterByClock(filteredTimes, latest, true);
                }
            } else if (mode == LoadMode.PREVIOUS_MODEL_RUN_2) {
                for (int i = 0; i < 2; i++) {
                    latest = filterByClock(filteredTimes, latest, true);
                }
            }

            // This is where the filtered times becomes empty
            if (filteredTimes.isEmpty()) {
                return loadTimes.toArray(new DataTime[loadTimes.size()]);
            }
            latest = makeRowloadList(filteredTimes, latest, loadTimes, false);
            ...
            // intentional fall thru to prog loop
            forecast = 0;
        }
        if (mode == LoadMode.PROG_LOOP || mode == LoadMode.ANALYSIS_LOOP) {
            ...
            filterByDeltaTime(loadTimes, deltaTime, preferred);
            makeFinalLoadList(loadTimes, nFrames, preferred, true);
    ...
    public DataTime[] makeOverlayList(DataTime[] depictTimes, Date clock,
        ...
        Date latest;
        List<DataTime> depictTimesList = new ArrayList<>(
        ...
        } else if (mode == LoadMode.PREVIOUS_MODEL_RUN_1) {
            latest = filterByClock(depictTimesList, clock, true);
            // Already found prev_run. Do it once more to get "prev run - 1"
            for (int i = 0; i < 1; i++) {
                latest = filterByClock(depictTimesList, latest, true);
            }
        } else if (mode == LoadMode.PREVIOUS_MODEL_RUN_2) {
            latest = filterByClock(depictTimesList, clock, true);
            // Already found prev_run. Do it twice more to get "prev run - 2"
            for (int i = 0; i < 2; i++) {
                latest = filterByClock(depictTimesList, latest, true);
            }
        } else {
        ...
        case PREVIOUS_MODEL_RUN_1:
        case PREVIOUS_MODEL_RUN_2:
    ...
    public static List<Long> computeTimeRes(int fcstTime, DataTime[] times,
        ...
        long minInterval = (new TimeMatcher()).intrinsicPeriod(times,
                haveForecasts).intrinsicPeriod;
        ...
        if (minInterval >= 40_000) {
            minInterval = (((minInterval + THREE_HOURS_S) / SIX_HOURS_S)
                    * SIX_HOURS_S);
        } else if (minInterval >= 3000) {
            minInterval = (((minInterval + HALF_HOUR_S) / ONE_HOUR_S)
                    * ONE_HOUR_S);
        } else {
            minInterval = (((minInterval + HALF_MINUTE_S) / ONE_MINUTE_S)
                    * ONE_MINUTE_S);
        }
        ...
        if (hasOffset) {
            if (intervals.get(m) < ONE_DAY_S) {
                intervals.add(ONE_DAY_S);
            }
            intervals.add(0, 0L);
            m = intervals.size() - 1;
            int a = 0;

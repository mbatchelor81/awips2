...
public class D2DTimeMatcher extends AbstractTimeMatcher {
    ...
    public void redoTimeMatching(IDescriptor descriptor) {
        synchronized (this) {
            ...
            Map<AbstractVizResource<?, ?>, DataTime[]> resourceTimeMap = new HashMap<>();
    ...
    private void populateConfiguration(TimeMatchingConfiguration config) {
        ...
        if (config.getClock() == null) {
            SimulatedTime sysTime = SimulatedTime.getSystemTime();
            DataTime[] times = config.getDataTimes();
            Long clockDelta = config.getClockDelta();
            if (sysTime.isRealTime() && (times != null) && (times.length != 0)
                    && clockDelta == null) {
                config.setClock(times[times.length - 1].getValidTimeAsDate());
            } else if (clockDelta == null) {
                config.setClock(sysTime.getTime());

            } else {
                config.setClock(new Date(sysTime.getMillis() - clockDelta));
            }
        }
    ...
    public List<ResourcePair> getResourceLoadOrder(
        ...
        if (basisPair != null) {
            List<ResourcePair> results = new ArrayList<>(resources);

 ...
 * May 26, 2016 5571       skorolev   {@link GetTotalPrecip} relocated to common.
...
public class PointDataControlDerive {
    ...
    private final List<GageData> dataList = new ArrayList<>();
    ...
    public void deriveRiverReports(boolean isOneTime, PDCOptions pcOptions) {
        ...
        if (((heightList == null) || (heightList.isEmpty()))
                ...
                && ((riverStatusList == null) || (riverStatusList.isEmpty()))) {
            return;
        }
         ...
         * the latest data.
         */
        if ((riverStatusList != null) && (!riverStatusList.isEmpty())) {
            List<Riverstatus> tempList = new ArrayList<>();
            ...
            for (int i = 0; i < riverStatusList.size(); i++) {
                 ...
                 * process
                 */
                if (!currentLid.equalsIgnoreCase(
                        riverStatusList.get(i).getId().getLid())) {
            ...
            if (pcOptions
                    .getTimeMode() == PDCConstants.TimeModeType.VALUE_CHANGE
                            .getTimeMode()) {
                changeWindow = getChangeHourWindow();
                changeWindow *= 3600;
                changeWindow /= 2;
            }
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    public List<GageData> deriveOtherReports() {
        ...
        List<Observation> obsList = pdcManager.getObservationList();
        ...
        List<Observation> tempObsList = new ArrayList<>();
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    private List<GageData> processLidObsRiver(List<Observation> obsList,
        ...
        if (bestObs != null) {
            ...
            oneRpt.setShefQualCode("Z");
            oneRpt.setQuality_code(PDCConstants.DEFAULT_QC_VALUE);
            if (PDCOptionData.getInstance().getTimeMode() == 
                    PDCConstants.TimeModeType.VALUE_CHANGE
                        .getTimeMode()) {
    ...
    private List<GageData> processLidRS(String[] useTs,
            List<Riverstatus> rsList, int startRecord, List<GageData> inputList,
            PDCOptions pcOptions) {
     ...
     * @param changeWindow
     */
    private void processLidObs(List<Observation> tempObsList,
            int changeWindow) {
        GageData oneRpt = new GageData();
        PDCOptionData pcOptions = PDCOptionData.getInstance();
                ...
                if ((pcOptions.getTimeMode() == TimeModeType.LATEST
                    ...
                    if (firstObsFound == 0) {
                        currentDiff = Math.abs(
                                pcOptions.getValidTime().getTime() - obsList
                                        .get(i).getObstime().getTime());
    ...
    private Observation deriveReportObsRiver(List<Observation> obsList,
        ...
        if (pcOptions.getPrimary() == 1) {
            ...
            RiverStat rsInfo = manager
                    .getRiverStatus(obsList.get(startRecord).getLid());
        ...
        for (int i = 0; i < obsList.size(); i++) {
            if (usePE.equals(obsList.get(i).getPe())
                ...
                if ((pcOptions.getTimeMode() == PDCConstants.TimeModeType.LATEST
                        .getTimeMode())
                        || (pcOptions
                                .getTimeMode() == PDCConstants.TimeModeType.MAXSELECT
                                        .getTimeMode())
                        || (pcOptions
                                .getTimeMode() == PDCConstants.TimeModeType.MINSELECT
                                        .getTimeMode())) {
                    /*
                     * for all but the SETTIME and VALUE_CHANGE modes, use the
                     * first value, which is the latest.
                    ...
                    return obsList.get(i);
                        ...
                        .getTimeMode() == PDCConstants.TimeModeType.SETTIME
                                .getTimeMode()) {
                	currentDiff = Math.abs(
                            pcOptions.getValidTime().getTime() - obsList.get(i)
                                    .getObstime().getTime());
     ...
     * @return
     */
    private Riverstatus deriveReportRS(String[] useTs, List<Riverstatus> rsList,
            int startRecord, PDCOptions pcOptions) {
        ...
        if (pcOptions.getPrimary() == 1) {
            ...
            /* try and find the pe specified for this location */
            rsInfo = manager.getRiverStatus(
                    riverStatusList.get(startRecord).getId().getLid());
        ...
        for (int i = 0; i < rsList.size(); i++) {
            ...
            if (usePE.equalsIgnoreCase(rsList.get(i).getId().getPe())) {
                for (int j = 0; j < useTs.length; j++) {
                    if (rsList.get(i).getId().getTs().startsWith(useTs[j])) {
                        match = true;
                        break;
                    }
                }
    ...
    public List<GageData> deriveRainReports() {
        ...
        PointDataControlManager pdcManager = PointDataControlManager
                .getInstance();
        List<GageData> reportList = new ArrayList<>();
        long beginTime;
        long endTime;
        ...
        List<Rawpc> rawPcList = new ArrayList<>();
        List<Rawpp> rawPpList = new ArrayList<>();
        if (pcList != null) {
            for (Curpc cp : pcList) {
        ...
        VizGetTotalPrecip vgtp = new VizGetTotalPrecip();
        reportList = vgtp.getTotalRawPrecip(rawPcList, rawPpList, beginDate,
    ...
    private int getChangeHourWindow() {

...
public class CloudHeightResource
        extends AbstractVizResource<CloudHeightResourceData, IMapDescriptor> {
     ...
     */
    private Quantity<?> modeCloudHeight;
    ...
    public synchronized String inspect(ReferencedCoordinate coord)
        ...
        if (sounding == null) {
            ...
            return "NO " + String.valueOf(soundingProvider.getSoundingSource())
                    .toUpperCase() + " DATA";
        }

        // Convert cloudHeight to feet and rount to nearest hundreds place
        int heightInFeet = ((int) (getDataValue(cloudHeight, USCustomary.FOOT)
                / 100 + 0.5) * 100);
        ...
        if (modeCloudHeight != null) {
            int otherHeightInFeet = ((int) (getDataValue(modeCloudHeight,
                    USCustomary.FOOT) / 100 + 0.5) * 100);
            if (otherHeightInFeet >= 0) {
                heightStr += "/" + otherHeightInFeet;
            }
        }

        return String.format("%s feet (%s %s)", heightStr,
                sounding.getStationId(),
                cloudHeightStatus != null ? cloudHeightStatus : "");
    }
    ...
    private void computeCloudHeightParameters(ReferencedCoordinate coord) {
        ...
        Quantity<?> cloudHeight = null;
        Quantity<?> modeCloudHeight = null;
        ...
        if (provider != null && location != null) {
            if (cloudHeightLocation != null
                    ...
                    && provider.getSoundingSource()
                            .equals(soundingProvider.getSoundingSource())) {
            ...
            if (contributingResource != null) {
                ...
                if (sounding != null) {
                    ...
                    if (sounding.size() == 0 && GridConstants.PLUGIN_NAME
                            .equals(provider.getSoundingSource())) {
                        ...
                        if (otherTempToUse != null) {
                            ...
                            otherHeight = CloudHeightCalculatorPorted
                                    .getCloudHeightClimo(
                                            tempToK.convert(otherTempToUse)
                                                    .floatValue(),
                    ...
                    if (height >= 0) {
                        cloudHeight = Quantities.getQuantity(height, SI.METRE);
                        if (otherHeight >= 0) {
                            modeCloudHeight = Quantities
                                    .getQuantity(otherHeight, SI.METRE);
                        }
                        temperature = Quantities.getQuantity(cloudTemp,
                                TEMP_UNIT);
                    }
                }
            }
    ...
    private float[] getTemperaturesForResource(
        ...
        if (timeForRsc != null) {
            try {
                ...
                if (interMap != null
                        && interMap
                                .get(SATELLITE_DATA_INTERROGATE_ID) instanceof Quantity
                        && interMap.containsKey(
                                IGridGeometryProvider.class.toString())) {
                    // Extract temperature values from the resource
                    float[] rscTemps = extractTemps(location, resource,
                            ((IGridGeometryProvider) interMap.get(
                                    IGridGeometryProvider.class.toString()))
                                            .getGridGeometry());
                    boolean good = true;
        ...
        if (latLon != null) {
            try {
                ...
                if (obj instanceof Quantity) {
                    temperature = getDataValue((Quantity<?>) obj,
     ...
     * (non-Javadoc)
     * 
     * @see com.raytheon.uf.viz.core.rsc.AbstractVizResource#paintInternal(com.

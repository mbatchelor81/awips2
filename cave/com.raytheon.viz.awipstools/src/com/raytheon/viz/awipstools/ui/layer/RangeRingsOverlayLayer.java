 ...
 *                                    Measure.                                  
 * Jun 26, 2019  21366    jdynina     Account for the possibility of below
 *                                    0 degree elevation values (tilts)
 *
 *
 *
 * </pre>
...
public class RangeRingsOverlayLayer implements IRenderable {
    ...
    private final UnitConverter convertFromMeterToKilometer = SI.METRE
            .getConverterTo(MetricPrefix.KILO(SI.METRE));

    private final UnitConverter convertFromMeterToFoot = SI.METRE
            .getConverterTo(USCustomary.FOOT);

    private final UnitConverter convertFromMileToKilometer = USCustomary.MILE
            .getConverterTo(MetricPrefix.KILO(SI.METRE));

    private final UnitConverter convertFromMileToMeter = USCustomary.MILE
    ...
    public void setRangedResource(IRangeableResource rangedResource)
        ...
        this.resourceSourceElevationInMeters = this.rangedResource
                .getElevation().to(SI.METRE).getValue().doubleValue();
    ...
    private double getMaxRingRadiusMiles(IExtent extent) {
        ...
        maxDistance = gc.getEllipsoid().getAxisUnit()
                .getConverterTo(USCustomary.MILE).convert(maxDistance);

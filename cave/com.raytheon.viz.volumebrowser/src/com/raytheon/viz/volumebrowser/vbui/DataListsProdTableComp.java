 ...
 * May 12, 2009  2161     lvenable  Initial creation
 * Jul 31, 2012  875      rferrel   Now uses markers.
 * Sep 26, 2012  1216     rferrel   Change listener added to update points menu.
 * Jan 14, 2013  1516     rferrel   Remove listeners on dispose and specify Data
 *                                  Selection in Points Tool Action.
 * Dec 06, 2013  2271     mpduff    Save the selected plane points so the menu's
 *                                  are recreated correctly on a pointChange
 *                                  action.
 * Dec 11, 2013  2602     bsteffen  Remove dead catch block.
 * Mar 18, 2014  2874     bsteffen  Move creation of Sources toolbar
 *                                  contributions to VbSourceList
 * Oct 03, 2014  2618     mapeters  Updated pointChangeListener's pointChanged()
 *                                  implementation to fully remove deleted
 *                                  points, added createUniqueKey(String,
 *                                  String, String).
 * Nov 18, 2015  4894     kbisanz   Call setActiveDataSelection(...) before
 *                                  calling splitToMultipleBars(...) to set data
 *                                  selection correctly and prevent menu items
 *                                  from being referenced on multiple menus.
 * Jan 12, 2016  5055     randerso  Added support for adjusting split of toolbar
 *                                  menus when dialog resized.
 * Apr 02, 2018  6765     bsteffen  Send all products to the product table to
 *                                  preserve order.
...
=======
        for (int element : sourcesSelIdx) {
            ...
            for (int element2 : fieldsSelIdx) {
                ...
                for (int element3 : planesSelIdx) {
                    uniqueKey = createUniqueKey(element, element2, element3);

                    selData = new SelectedData(
                            sourceControl.list.getItemText(element),
                            sourceControl.list.getKey(element),
                            fieldControl.list.getItemText(element2),

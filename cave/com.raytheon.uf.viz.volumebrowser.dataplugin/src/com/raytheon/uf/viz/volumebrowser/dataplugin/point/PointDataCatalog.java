...
public class PointDataCatalog extends AbstractInventoryDataCatalog {
    ...
    public List<String> getSupportedSources() {
        ...
        for (String supportedSource : super.getSupportedSourcesInternal()) {
            supportedSources.add(supportedSource);
            supportedSources.add(supportedSource + "OA");
        }
        supportedSources.retainAll(MenuItemManager.getInstance()
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    @Override
    protected Set<String> getAllPossibleSources() {
        Set<String> sources = new HashSet<>();
        for (String source : super.getAllPossibleSources()) {
            if (source.endsWith("OA")) {
                sources.add(source.substring(0, source.length() - 2));
            } else {
                sources.add(source);
            }
        }

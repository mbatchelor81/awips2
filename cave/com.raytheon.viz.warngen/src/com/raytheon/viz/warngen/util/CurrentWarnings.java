 ...
 * Feb 22, 2021 8258       mapeters     Replace cases where we pull full warning records right after pulling
 *                                      data URIs with a single request for full records
 * </pre>
 *
 * @author mschenke
 */
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    private static final Map<String, CurrentWarnings> instanceMap = new HashMap<>();
     ...
     * @return
     */
    public static synchronized CurrentWarnings getInstance(
            String localizedSite) {
        CurrentWarnings warnings = instanceMap.get(localizedSite);
        if (warnings == null) {
            warnings = new CurrentWarnings(
                    SiteMap.getInstance().getSite4LetterId(localizedSite));
    ...
    private final Map<String, AbstractWarningRecord> recordsMap = new HashMap<>();
    ...
    private CurrentWarnings(String officeId) {
        ...
        SimulatedTime.getSystemTime()
                .addSimulatedTimeChangeListener(changeListener);
    }

    /**
     * request and populate data
     */
    private void initializeData(boolean fullRecords) {
        ...
        constraints.put("issueTime", new RequestConstraint(
                TimeUtil.formatDate(time), ConstraintType.LESS_THAN_EQUALS));
    ...
    public List<AbstractWarningRecord> getCurrentWarnings() {
        List<AbstractWarningRecord> rval = new ArrayList<>();

        synchronized (lock) {
            List<WarningKey> keys = new ArrayList<>(warningMap.keySet());
            for (WarningKey key : keys) {
                AbstractWarningRecord tmp = getNewestByTracking(key.etn,
                        key.phensig);
                if (tmp != null && !rval.contains(tmp)) {
                    rval.add(tmp);
                }
            }
        }
    ...
    public List<AbstractWarningRecord> getCorrectableWarnings(
        ...
        synchronized (lock) {
            List<AbstractWarningRecord> records = warningMap
                    .get(toKey(warnRec.getPhensig(), warnRec.getEtn()));
            for (AbstractWarningRecord warning : records) {
                ...
                if (((action == WarningAction.NEW)
                    ...
                    for (AbstractWarningRecord otherWarning : records) {
                        ...
                        if ((otherAction == WarningAction.NEW
                                || otherAction == WarningAction.CON)
                                && warning.getUgcZones()
                                        .equals(otherWarning.getUgcZones())) {
                            rval.clear();
                            return rval;
                        }
                    }
    ...
    public AbstractWarningRecord getNewestByTracking(String etn,
            String phensig) {
        AbstractWarningRecord rval = null;
        synchronized (lock) {
            List<AbstractWarningRecord> keyWarnings = warningMap
                    .get(toKey(phensig, etn));
            if (keyWarnings != null) {
                ...
                for (AbstractWarningRecord warning : warnings) {
                    ...
                    if ((action == WarningAction.CAN)
                        ...
                        if ((rval != null) && (warning.getCountyheader()
                                .equals(rval.getCountyheader())
                                || warning.getUgcZones()
                                        .containsAll(rval.getUgcZones()))) {
                            rval = null;
                        }
                    }
                }
    ...
    public AbstractWarningRecord getFollowUpByTracking(String etn,
        ...
        AbstractWarningRecord rval = null;
        synchronized (lock) {
            List<AbstractWarningRecord> warnings = warningMap
                    .get(toKey(phensig, etn));
            if (warnings != null) {
    ...
    public AbstractWarningRecord getCancelledByTracking(String etn,
        ...
        synchronized (lock) {
            ...
            if (warnings != null) {
                ...
                ArrayList<AbstractWarningRecord> conProds = new ArrayList<>();
                Calendar c = TimeUtil.newCalendar();
                c.add(Calendar.MINUTE, -10);
                TimeRange t = new TimeRange(c.getTime(), TimeUtil.newDate());
                for (AbstractWarningRecord warning : warnings) {
    ...
    private List<AbstractWarningRecord> prepare(
        ...
        List<AbstractWarningRecord> prepared = new ArrayList<>();
        ...
        if (!dataURIs.isEmpty()) {
            ...
            try {
                List<AbstractWarningRecord> toProcess = new ArrayList<>();
    ...
    private void processRecords(List<AbstractWarningRecord> warnings) {
        synchronized (lock) {
            ...
            List<AbstractWarningRecord> records = new ArrayList<>(
                    recordsMap.values());
            ...
            Map<WarningKey, List<AbstractWarningRecord>> tmpMap = new HashMap<>();
            for (AbstractWarningRecord record : records) {
    ...
    private static void alertArrived(Collection<AlertMessage> alertMessages) {
        List<String> dataURIs = new ArrayList<>(alertMessages.size());
        for (AlertMessage am : alertMessages) {
            if (instanceMap.containsKey(
                    String.valueOf(am.decodedAlert.get("xxxid")))) {
        ...
        Map<String, List<AbstractWarningRecord>> recordMap = new HashMap<>();
        ...
        for (Entry<String, List<AbstractWarningRecord>> entry : recordMap
                .entrySet()) {
            String key = entry.getKey();
            CurrentWarnings cw = instanceMap.get(key);
            if (cw != null) {
                List<AbstractWarningRecord> records = entry.getValue();
                cw.processRecords(records);
            }
        }
    ...
    private static List<AbstractWarningRecord> requestRecords(
            Map<String, RequestConstraint> constraints, boolean fullRecords) {
        Map<String, RequestConstraint> constraintsCopy = new HashMap<>(
                constraints);
        // Ensures we won't request any records we wont use
        constraintsCopy.put("geometry",
                new RequestConstraint(null, ConstraintType.ISNOTNULL));
        List<AbstractWarningRecord> newRecords = new ArrayList<>();
        ...
        try {
            ...
            DbQueryResponse response = (DbQueryResponse) ThriftClient
                    .sendRequest(request);
            if (fullRecords) {
                AbstractWarningRecord[] records = response
                        .getEntityObjects(AbstractWarningRecord.class);
                newRecords.addAll(Arrays.asList(records));
            } else {
                for (Map<String, Object> result : response.getResults()) {
                    ...
                    record.setInsertTime(
                            (Calendar) result.get(insertTimeField));

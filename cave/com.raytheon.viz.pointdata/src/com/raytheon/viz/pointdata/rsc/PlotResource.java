 ...
 * Jan 07, 2020  73083    ksunil    added frameMap.clear between successive invocations of filter apply, .svg 
 *                                   refresh.
...
public class PlotResource
    ...
    public PlotResource(PlotResourceData data, LoadProperties props) {
        ...
        String plotModelPath = SVGImageFactory.PLOT_MODEL_DIR
                + IPathManager.SEPARATOR + data.getPlotModelFile();
        PathManagerFactory.getPathManager().addLocalizationPathObserver(
                plotModelPath, new ILocalizationPathObserver() {

                    @Override
                    public void fileChanged(ILocalizationFile file) {
                        PlotResource.this.recycle();
                    }
                });
    }
    ...
    private void initGenerator() throws VizException {
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    public void setConditionalFilter(Condition filter) {
        if (generator != null) {
            generator.setFilterCondition(filter);
            clearImages();
            /*
             * frameMap.clear() is necessary. Otherwise, the filtered set from
             * the last "Apply" seems to stick. So, you can't go from a more
             * restrictive (less items to display) to a less restrictive (more
             * items) filter.
             */
            frameMap.clear();
        }
    ...
    public void modelGenerated(PlotInfo[] key, IImage image) {
        ...
        if (frameInfo != null) {
            ...
            if (s != null) {
                if (image != null) {
                    s.plotImage = new PointImage(image, s.pixelLocation);

...
public class AbstractRadarResource<D extends IDescriptor>
    ...
    public void modifyTimeMatching(D2DTimeMatcher d2dTimeMatcher,
        ...
        synchronized (radarBasis.radarRecords) {
    ...
    public static long intervalCalc(AbstractRadarResource<?> radarRsc) {
        try {
            // cached DataTimes
            DataTime[] radarRscTimes = radarRsc.getResourceData()
                    .getAvailableTimes();

            Optional<Long> volumeScanTime = intervalCalcHelper(radarRscTimes);

            /*
             * return the calculated volumeScanTime, or the default if the time
             * is null
             */
            return volumeScanTime.orElse(5 * TimeUtil.MILLIS_PER_MINUTE);
        } catch (Exception e) {
            statusHandler.handle(Priority.PROBLEM,
                    "Exception occurred during calculation of volume scan time interval.",
                    e);
        }
     ...
     *
     * This method is to ensure the radar records are synchronized properly.
     *
     * @param <T>
     *            the return type
     * @param processor
     *            the processing function to apply to the radar records
     * @return the processing result
     */
    public <T> T processRadarRecords(
            Function<Map<DataTime, VizRadarRecord>, T> processor) {
        synchronized (radarRecords) {

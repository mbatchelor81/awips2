...
def sortHazardList(dict):
    ...
    #dictionary
    import VTECTable
    for (ent, values) in dict.items():
        # get the descriptive word for this phen/sig
        items = sorted([(VTECTable.VTECTable.get(v,{}).get('hdln', ''), v) for v in values])
        #extract out the sorted phen/sig
        dict[ent] = [v for (desc, v) in items]
...
hazardDict = OrderedDict([
    ...
    ('Non-Precipitation', ["AF.W", "AF.Y", "AQ.Y", "AS.O", "AS.Y", "CW.Y",
          "DU.W", "DU.Y", "EC.W", "EC.A", "FG.Y", "FZ.W",
          "FZ.A", "HZ.W", "HZ.A", "ZF.Y", "FR.Y", "HT.Y", "HW.W", "HW.A",
          "LW.Y", "SM.Y", "WI.Y", "XH.W", "XH.A"]),

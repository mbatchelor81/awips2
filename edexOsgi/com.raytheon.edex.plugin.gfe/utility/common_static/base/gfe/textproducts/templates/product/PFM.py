...
class TextProduct(TextRules.TextRules, SampleAnalysis.SampleAnalysis):
    ...
    def generateForecast(self, argDict):
        ...
        tzDictTR = {}
        for (key, value) in self._areaTZ.items():
            tz = value
        ...
        for (timeZone, value) in tzDictTR.items():
            areasInTimeZone = []
            ...
            sampleInfo += self._sampleData(argDict, value, areasInTimeZone)
...
Found description: """ + areaLabel)
    ...
    def _hour24zuluLabel(self, timeRange):
        ...
        return label.rjust(self._top3hrWidth)
    ...
    def _tempValue(self, statDict, timeRange, argList):
        ...
        if val is None:
            return "MM"
        return repr(int(self.round(val, "Nearest", 1)))
    ...
    def _rhValue(self, statDict, timeRange, argList):
        ...
        if tdval is None:
            return "MM"
        t = self.round(tval, "Nearest", 1)
        td = self.round(tdval, "Nearest", 1)
        ...
        return repr(int(self.round(RH, "Nearest", 1)))
    ...
    def _qpfValue(self, statDict, timeRange, argList):
        ...
        if maxV - minV > self._qpfRangeThreshold and self._productType == "AFM":
            minString = '{:5.2f}'.format(minV).strip()
            maxString = '{:5.2f}'.format(maxV).strip()
            return minString + "-" + maxString
        elif sumV < 0.005:
            return "0"
        else:
            return '{:5.2f}'.format(sumV).strip()
    ...
    def _snowValue(self, statDict, timeRange, argList):
        ...
        if maxV - minV > self._snowRangeThreshold and \
          self._productType == "AFM":
            minString = repr(int(self.round(minV, "Nearest", 1)))
            maxString = repr(int(self.round(maxV, "Nearest", 1)))
            return minString + "-" + maxString
        elif sumV < 0.1:
            return "00-00"
        elif sumV < 0.5:
            ...
            return repr(int(self.round(sumV, "Nearest", 1)))
    ...
    def _mxmnValue(self, statDict, timeRange, argList):
        ...
        if type(val) is tuple:
            ...
            if maxV - minV > self._tempRangeThreshold and \
              self._productType == "AFM":
                minString = repr(int(self.round(minV, "Nearest", 1))).rjust(self._top3hrWidth)
                aveString = repr(int(self.round(aveV, "Nearest", 1))).rjust(self._top3hrWidth)
                maxString = repr(int(self.round(maxV, "Nearest", 1))).rjust(self._top3hrWidth)
                return minString + aveString + maxString
            else:
                return repr(int(self.round(aveV, "Nearest", 1)))
        else:
            return repr(int(self.round(val, "Nearest", 1)))
    ...
    def _windChillValue(self, statDict, timeRange, argList):
        ...
        chill = self.getStats(statDict, "ApparentT")
        ...
        if chill < temp and chill < self._windChillLimit and \
           (temp - chill) > self._windChillDifference:
            return repr(int(self.round(chill, "Nearest", 1)))
        else:
            return ""
    ...
    def _heatIndexValue(self, statDict, timeRange, argList):
        ...
        heat = self.getStats(statDict, "ApparentT")
        temp = self.getStats(statDict, "T")
        ...
        if heat >= self._heatIndexLimit and \
           (heat - temp) >= self._heatIndexDifference:
            return repr(int(self.round(heat, "Nearest", 1)))
        else:
            return ""
    ...
    def _minWindChillValue(self, statDict, timeRange, argList):
        ...
        if minV < self._windChillLimit:
            return repr(int(self.round(minV, "Nearest", 1)))
        else:
            return ""
    ...
    def _maxHeatIndexValue(self, statDict, timeRange, argList):
        ...
        if maxV >= self._heatIndexLimit:
            return repr(int(self.round(maxV, "Nearest", 1)))
        return ""
    ...
    def _windGustValue(self, statDict, timeRange, argList):
        ...
        speed = self.round(maxGust, "Nearest", 1) * 1.15  # convert to MPH
    ...
    def _windValue(self, statDict, timeRange, argList):
        ...
        if windType == "dir":
            ...
            speed = self.round(mag, "Nearest", 1) * 1.15  # convert to MPH
            return repr(int(speed))
    ...
    def _createWxRows(self, fcst, periods, sampler, analysisList,
        ...
        for wxVal in sorted(wxDict.keys()):
            if wxVal == "":
                continue
    ...
    def _createWWARows(self, fcst, periods, editAreaList, colWidth):
        ...
        for wwaVal in sorted(wwaDict.keys()):
    ...
    def _createObVisRows(self, fcst, periods, sampler, analysisList,
        ...
        for x in range(len(obvisValues)):
            if type(colWidth) is list:
                ...
                width = colWidth
            fcst += obvisValues[x].rjust(width)
        fcst += "\n"
        return fcst
    ...
    def _calcPeriodLabels(self, periods, colWidth, startPoint, intervalHours,
      timeZone):
        ...
        lt = ltZone.ljust(4) + repr(intervalHours) + "hrly "
        lt = lt.ljust(startPoint)

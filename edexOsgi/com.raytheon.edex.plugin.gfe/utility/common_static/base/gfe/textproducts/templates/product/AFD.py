...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
        ...
        for index in range(body_start_index, end_index):
            if re.match(WWABlockString, oldAFD[index]) is not None:
        ...
        if prelim_divider:
            for index in range(body_start_index, body_end_index):
                if re.match(prelim_divider, oldAFD[index]) is not None:
                    body_end_index = index
                    break
...
####################################################################
    def _getTemp(self, value):
        ...
        value = int(self.round(value, "Nearest", 1))
        valStr = repr(value).rjust(3)
        return valStr

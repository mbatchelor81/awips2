...
public abstract class AbstractBufrSfcObsDecoder {
    ...
    public static final String localizationTablesDirectory = "bufrobs"
            + IPathManager.SEPARATOR + "tables";

    private final Map<String, TranslationTable> translationMap = new HashMap<>();
    ...
    private static Map<String, ParameterDescription> createParamDefMap(
        ...
        Map<String, ParameterDescription> map = new HashMap<>(params.length);
    ...
    public PluginDataObject[] decode(BufrParser parser, CategoryKey key)
        ...
        List<PluginDataObject> rval = new ArrayList<>();
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    protected void processLocationField(SurfaceObsLocation location,
        ...
        if (baseName.equalsIgnoreCase(SfcObsPointDataTransform.STATION_ID)) {
            ...
            Number elevation = getInUnits(parser, SI.METRE);
            if (elevation != null) {
     ...
     * @param obj
     * @param toUnit
     * @return If unit conversion is successful, returns value of converted
     *         units. If unsuccessful, returns null.
     * @throws BufrObsDecodeException
     */
    protected Number getInUnits(Variable var, Object obj, Unit<?> toUnit)
        ...
        if (obj instanceof Number) {
            ...
            try {
                converter = fromUnit.getConverterToAny(toUnit);
                rval = converter.convert(value.doubleValue());
            } catch (IncommensurableException | UnconvertibleException e) {
                log.error("Unable to convert units from " + fromUnit.getName()
                        + " to " + toUnit.getName(), e);
                rval = null;
            }
<<<<<<< HEAD
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    protected Object getParameterValue(ParameterDescription desc,
        ...
        if (desc.getUnit() != null) {
            toUnit = SimpleUnitFormat.getInstance()
                    .parseProductUnit(desc.getUnit(), new ParsePosition(0));
        }
        Object value;
        if (toUnit != null) {
    ...
    protected void processPrecip(ObsCommon record, BufrParser parser)
            throws BufrObsDecodeException {
        Number precip = getInUnits(parser, MetricPrefix.MILLI(SI.METRE));
        if (precip == null) {
            /* missing value */
            return;
        }

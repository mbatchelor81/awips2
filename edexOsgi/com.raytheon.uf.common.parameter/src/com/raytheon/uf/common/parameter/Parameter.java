...
public class Parameter extends PersistableDataObject implements
    ...
    public Unit<?> getUnit() {
        if (unit == null) {
            if (unitString != null && unitString.length() > 0) {
                try {
                    unit = (Unit<?>) SimpleUnitFormat.getInstance(SimpleUnitFormat.Flavor.ASCII).parse(
                            unitString);
    ...
    public void setUnit(Unit<?> unit) {
        this.unit = unit;
        if (unit == null || unit.equals(AbstractUnit.ONE)) {
            this.unitString = "";
        } else {
            this.unitString = SimpleUnitFormat.getInstance(SimpleUnitFormat.Flavor.ASCII).format(unit);
        }
    }
    ...
    public static UnitConverter compareUnits(Parameter sourceParameter,
        ...
        if (srcString == null) {
            ...
            try {
                return srcUnit.getConverterToAny(destUnit);

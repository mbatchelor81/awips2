...
class JGridData(IGridData, JData.JData):
    ...
    def getRawData(self, unit=None):
        ...
        if unit:
            ...
            unitObj = SimpleUnitFormat.getInstance(SimpleUnitFormat.Flavor.ASCII).parseObject(unit, ParsePosition(0))
            converter = self.jobj.getUnit().getConverterTo(unitObj)
            dataFilter = UnitConvertingDataFilter(converter)
            filters = jarray(1, UnitConvertingDataFilter)
            filters[0] = dataFilter

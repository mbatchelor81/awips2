...
public class VisibilityParser {
    ...
    private static final UnitConverter METERS_TO_MILES = SI.METRE
            .getConverterTo(USCustomary.MILE);

    private static final UnitConverter KM_TO_MILES = MetricPrefix.KILO(SI.METRE)
            .getConverterTo(USCustomary.MILE);

    private static final UnitConverter MILES_TO_METERS = USCustomary.MILE
    ...
    public boolean decode(String visString) {
        ...
        if((offset = vis[0].indexOf("KM")) != -1) {
            ...
            if(km.matches()) {
                this.prevail_vsbySM = KM_TO_MILES.convert(prevailVSBY(vis[0]).doubleValue());
                this.visibility = vis[0];
                return true;
            }
            else {

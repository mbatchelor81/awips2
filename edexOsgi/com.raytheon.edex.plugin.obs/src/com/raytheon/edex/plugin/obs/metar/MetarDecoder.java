...
public class MetarDecoder {
    ...
    /** Regular expression for 1-hour ice accumulation */
    public static final Pattern ICEACCUM_1HR_EXP = Pattern
            .compile("\\bI1(\\d{3})\\b");

    /** Regular expression for 3-hour ice accumulation */
    public static final Pattern ICEACCUM_3HR_EXP = Pattern
            .compile("\\bI3(\\d{3})\\b");

    /** Regular expression for 6-hour ice accumulation */
    public static final Pattern ICEACCUM_6HR_EXP = Pattern
            .compile("\\bI6(\\d{3})\\b");
        ...
        while (sep.hasNext()) {
            ...
            try {
                ...
                if (matcher.find()) {
                    ...
                    String obsTimeGroup = matcher.group(3);
                    Calendar obsTime = WMOTimeParser.getAdjustedRefTime(
                            obsTimeGroup, baseTime);
                    if (obsTime != null) {
                        record.setTimeObs(obsTime);
                        record.setDataTime(new DataTime(obsTime));
                        Calendar refHour = TimeTools.copyToNearestHour(obsTime);
                        if (obsTime.get(Calendar.MINUTE) >= 45) {
                ...
                if (matcher.find()) {
                    ...
                    if (matcher.group(1).equals("NO")) {
                        ...
                        if (slp != null) {
                            slp = MetricPrefix.HECTO(SI.PASCAL)
                ...
                // Get the ice accumulation over the last hour
                matcher = ICEACCUM_1HR_EXP.matcher(trailingData);
                if (matcher.find()) {
                    String s = matcher.group(1).trim();
                    try {
                        float iceAccum = Integer.parseInt(s) / 100.0f;
                        record.setIceAccum1Hour(iceAccum);
                    } catch (NumberFormatException nfe) {
                        logger.error(
                                traceId + " -Error decoding iceAccum " + s);
                    }
                }
                // Get the ice accumulation over the last 3 hours
                matcher = ICEACCUM_3HR_EXP.matcher(trailingData);
                if (matcher.find()) {
                    String s = matcher.group(1).trim();
                    try {
                        float iceAccum = Integer.parseInt(s) / 100.0f;
                        record.setIceAccum3Hour(iceAccum);
                    } catch (NumberFormatException nfe) {
                        logger.error(
                                traceId + " -Error decoding iceAccum " + s);
                    }
                }
                // Get the ice accumulation over the last 6 hours
                matcher = ICEACCUM_6HR_EXP.matcher(trailingData);
                if (matcher.find()) {
                    String s = matcher.group(1).trim();
                    try {
                        float iceAccum = Integer.parseInt(s) / 100.0f;
                        record.setIceAccum6Hour(iceAccum);
                    } catch (NumberFormatException nfe) {
                        logger.error(
                                traceId + " -Error decoding iceAccum " + s);
                    }

...
%(prog)s -h hostname -r port -d outputDatabaseID [-p parm] -f inputFile
  ...
  [-a altMask] [-w messagePrefix] [-D gridDelay] [-o] [-A] [-y]"""
    ...
    parser.add_argument("-A", action="store", dest="additionalRoutingSiteID",
                      help="Apply AdditionalIscRouting for the source siteID",
                      metavar="additionalRoutingSiteID")
    parser.add_argument("-y", action="store_true", dest="asynchronous",
                      help="Do not wait for processing to complete")
...
def create_request(host, port, userID, databaseID, parmsToProcess, blankOtherPeriods, 
    ...
    obj = ExecuteIscMosaicRequest(userID, databaseID, parmsToProcess, 
            ...
            additionalRoutingSiteID, asynchronous)
    return obj

...
def officialDB(dbId, officialDBs):
    for odb in officialDBs:
        if dbId.getSiteId() == odb.getSiteId() and \
           dbId.getDbType() == odb.getDbType() and \
           dbId.getFormat() == odb.getFormat() and \
           dbId.getModelId() == odb.getModelId():
            return True
    return False
...
def breakAllLocksGRIDRequest(officialDBs, lockTables, databaseIDs, allLocks):
    ...
    for lt in lockTables:
        if officialDB(lt.getParmId().getDbId(), officialDBs):
            continue
        if not allLocks and repr(lt.getParmId().getDbId()) not in databaseIDs:
            continue
...
def validateArgs():
    parser = UsageArgumentParser.UsageArgumentParser(
        prog='ifpBreakAllLocks', conflict_handler="resolve")
    parser.add_argument(
        "-h",
        action="store",
        dest="host",
    ...
    if not options.allLocks:
        ...
        if invalidDbIds:
            parser.error(
                "Invalid DatabaseIDs specified: {}".format(invalidDbIds))
...
def main():
    ...
    log.info('allLocks= %s, Ids= %s', options.allLocks, options.databaseIDs)

    thriftClient = ThriftClient.ThriftClient(
        options.host, options.port, "/services")
    ...
    if options.siteID and options.siteID in activeSites:
        ...
        if options.siteID and options.siteID not in activeSites:
            log.error(
                "Invalid site ID %s specified, only sites %s are valid",
                options.siteID,
                activeSites)
        else:
            log.error(
                "Must use the -s option to specify one of the following sites %s",
                activeSites)
        sys.exit(1)

    try:
        officialDbNamesRequest = getOfficialDbNamesRequest(siteID)
        officialDbNameResponse = thriftClient.sendRequest(
            officialDbNamesRequest)
    except Exception:
        log.exception("Unable to retrieve official databases:")
        sys.exit(1)
    if not officialDbNameResponse.isOkay():
        log.error(
            "Unable to retrieve official databases: %s",
            officialDbNameResponse.message())
        sys.exit(1)
    ...
    if (not lockTableResponse.isOkay()):
        log.error(
            "Unable to retrieve lock table: %s",
            lockTableResponse.message())
        sys.exit(1)
    lockTables = lockTableResponse.getPayload()

    breakRequest = breakAllLocksGRIDRequest(
        officialDBs, lockTables, options.databaseIDs, options.allLocks)
    if breakRequest:
        try:

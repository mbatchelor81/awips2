...
#                                                 code cleanup.
#    05/03/19        #7842         dgilling       Update for netcdf4-python.
#
##
...
def __parseCdfFile(cdfFile):
    ...
    with netCDF4.Dataset(cdfFile, 'r') as cdfObj:        
        # getting times
        # ported from A1 D2DFile.getTimes()
        try:
            timesVar = cdfObj.variables['valtimeMINUSreftime']
            if timesVar.datatype.char != 'i':
                raise ValueError
            times = sorted(timesVar[:].tolist())
...
def __getParmAtts(ncVar, varName, ncFile):
    ...
    try:
        ...
        attrMap['fillValue'] = float(fillValue)
    except AttributeError:
    ...
    try:
        attrMap['levelsDesc'] = ncVar.levels
    except AttributeError:
    ...
    try:
        ...
        parmLevels = []
        for charString in rawLevels[:]:
            try:
                # some levels are returned as numpy MaskedArrays
                charString = charString.data
            except AttributeError:
                pass
            levelString = ''.join(c for c in charString if c.isalnum())
            if len(levelString):

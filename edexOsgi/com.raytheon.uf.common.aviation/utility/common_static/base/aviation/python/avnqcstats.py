...
logging.basicConfig(filename='/tmp/avnqcstats-' + dateStr + '.log', level=logging.DEBUG)
...
def genFiles(siteList, monthList, climateDir, listener=None):
    totstart = time.time()
    ...
    for ident in sites :
        for month in months:
            try:
                if listener and listener.isCanceled():

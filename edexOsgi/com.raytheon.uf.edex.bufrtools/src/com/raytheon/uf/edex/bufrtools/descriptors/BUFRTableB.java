 ...
 * 09/16/2014    #3628     mapeters    Moved from uf.edex.decodertools plugin.
 * 04/15/2019    7596      lsingh      Added units that removed during JSR-363 upgrade
 * 
 * </pre>
 * 
 * @author jkorman
...
public class BUFRTableB extends BUFRDescriptor {
    ...
    static {
        UNITS_MAP.put("%", Units.PERCENT);

        UNITS_MAP.put("DEG K", SI.KELVIN);
        UNITS_MAP.put("DEGREES KELVIN", SI.KELVIN);
        UNITS_MAP.put("DEG K/SEC", SI.KELVIN.divide(SI.SECOND));

        UNITS_MAP.put("M", SI.METRE);
        UNITS_MAP.put("M**2", SI.METRE.pow(2));

        UNITS_MAP.put("M/SEC", Units.METRE_PER_SECOND);
        UNITS_MAP.put("METERS/SECOND", Units.METRE_PER_SECOND);
        UNITS_MAP.put("M/SEC**2", SI.METRE_PER_SQUARE_SECOND);

        UNITS_MAP.put("M**2/SEC", SI.METRE.pow(2).divide(SI.SECOND));
        // Geopotential
        UNITS_MAP.put("M**2/SEC**2", SI.METRE.pow(2).divide(SI.SECOND.pow(2)));
        UNITS_MAP.put("M**3/SEC", SI.METRE.pow(3).divide(SI.SECOND));

        UNITS_MAP.put("DEG", NonSI.DEGREE_ANGLE);
        UNITS_MAP.put("°", NonSI.DEGREE_ANGLE);
        ...
        UNITS_MAP.put("KG/M**2", SI.KILOGRAM.divide(SI.METRE.pow(2)));
        UNITS_MAP.put("KG/M**2/SEC", SI.KILOGRAM.divide(SI.METRE.pow(2).divide(
                SI.SECOND)));
        UNITS_MAP.put("KG/M**3", SI.KILOGRAM.divide(SI.METRE.pow(3)));

        UNITS_MAP.put("J/M**2", SI.JOULE.divide(SI.METRE.pow(2)));
        ...
        UNITS_MAP.put("DOBSON", MetricPrefix.MILLI(SI.METRE).divide(100));

        UNITS_MAP.put("DB", CustomUnits.DECIBEL);
        UNITS_MAP.put("DB/M", CustomUnits.DECIBEL.divide(SI.METRE));

        UNITS_MAP.put("BQ", SI.BECQUEREL);
        UNITS_MAP.put("BQ/L", SI.BECQUEREL.divide(Units.LITRE));
        UNITS_MAP.put("BQ/M**3", SI.BECQUEREL.divide(SI.METRE.pow(3)));
        UNITS_MAP.put("MILLI-SIEV", SI.SIEVERT.divide(1000));

        // Vorticity | Divergence
        UNITS_MAP.put("1/SEC", AbstractUnit.ONE.divide(SI.SECOND));
        UNITS_MAP.put("PARTS/1000", AbstractUnit.ONE.divide(1000));

        UNITS_MAP.put("MIN", Units.MINUTE);
        UNITS_MAP.put("HOURS", Units.HOUR);

        UNITS_MAP.put(UNITS_NUMERIC, AbstractUnit.ONE);
        UNITS_MAP.put(UNITS_FLAGTABLE, AbstractUnit.ONE);
        UNITS_MAP.put(UNITS_CODETABLE, AbstractUnit.ONE);
    }

 ...
 * May 27, 2021 84655      Robert.Blum    Change name to hostName.
 * Jul 15, 2021 88368      Gang Chen      Added the getPurgeTimePeriod() method
 * Aug 09, 2021 84654      Robert.Blum    Added getter for localDomains.
 * Aug 30, 2021 93179      Amanuel Challa Added getEdexVersion() getter for edex cutoff version to.
 *
 * </pre>
 *
 * @author tgurney
...
public class BackupServiceConfigManager {
    ...
    public List<String> getHostnamesOnly() {
        return config.getHosts().stream().map(BackupHost::getHostName)
                .collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    public int getPurgeTimePeriodDays() {
        return config.getPurgeTimePeriodDays();
    }

    public String getEdexCutoffVersion() {
        return config.getEdexCutoffVersion();
    }

    public List<BackupServiceFile> getIncludeList() {
        return config.getIncludeFileList();
    }

    public List<BackupServiceFile> getExcludeList() {
        return config.getExcludeFileList();
    }

    public List<String> getLocalDomains() {
        return config.getLocalDomains();
    }
}

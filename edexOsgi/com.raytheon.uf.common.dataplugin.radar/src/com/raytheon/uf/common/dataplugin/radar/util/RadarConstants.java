 ...
 * May 28, 2014  2990     bsteffen  Fix radar mesocyclone regex.
...
public class RadarConstants {
    ...
    public enum MapValues {
        ...
        HAIL_TYPE,
        STORM_ID,
        DMD_ELEV_TIMES("elev_time"),
        DMD_ELEV_ANGLES("elev_angle"),
        DMD_LAST_ELEV("last_elev_flag"),
        DMD_AVG_SPD("avg_spd"),
        DMD_AVG_DIR("avg_dir"),
        HI_MAX_HAIL_SIZE,
        STI_AVG_SPEED,
        ...
        HI_POSH,
        HI_POH,
        STI_AZIMUTH_DIRECTION,
        STI_AZIMUTH_RANGE,
        STI_MOVEMENT_DIRECTION,
        STI_MOVEMENT_SPEED,
        STI_FORECAST_15_DIRECTION,
        STI_FORECAST_15_RANGE,
        STI_FORECAST_30_DIRECTION,
        STI_FORECAST_30_RANGE,
        STI_FORECAST_45_DIRECTION,
        STI_FORECAST_45_RANGE,
        STI_FORECAST_60_DIRECTION,
        STI_FORECAST_60_RANGE,
        STI_ERROR_FCST,
        STI_ERROR_MEAN,
        STI_DEF_DIR,
        STI_DEF_SPD,
        ...
        VAD_SRNG,
        VAD_ELEV,
        VAD_ANALYSIS_SLANT_RNG,
        VAD_BEGIN_AZIMUTH_ANGLE,
        VAD_END_AZIMUTH_ANGLE,
        VAD_NUM_PASSES,
        VAD_RMS_THRESHOLD,
        TVS_FEATURE_TYPE,
        TVS_AZIMUTH,
        ...
        TVS_CIRC_RADIUS_2,
        TVS_VECTOR_VEL_DIFF,
        TVS_MAX_PATTERN_VEC_RANGE,
        TVS_CIRC_RADIUS_RANGE,
        TVS_MAX_PATTER_VECTOR_HEIGHT,
        TVS_MAX_2D_FEATURES,
        TVS_MAX_NUM_PATTERN_VECTORS,
        TVS_MIN_2D_FEATURES,
        TVS_DIFF_VEL_1,
        TVS_DIFF_VEL_2,
        TVS_DIFF_VEL_3,
        ...
        TVS_DIFF_VEL_5,
        TVS_DIFF_VEL_6,
        TVS_MIN_3D_FEATURE_DEPTH,
        TVS_MIN_3D_FEATURE_LOW_LVL_DELTA,
        TVS_MIN_TVS_DELTA_VEL,
        TVS_MAX_3D_FEATURES,
        TVS_MAX_TVS,
        TVS_MAX_ELEVATED_TVS,
        TVS_MIN_VECTORS,
        TVS_MIN_TVS_BASE_HEIGHT,
        TVS_2D_VECTOR_RADIAL_DISTANCE,
        TVS_MIN_TVS_ELEVATION,
        TVS_2D_VECTOR_AZIMUTHAL_DIST,
        TVS_MIN_AVG_DELTA_VEL_HEIGHT,
        TVS_2D_FEATURE_ASPECT_RATIO,
        TVS_MAX_STORM_ASSOCIATION_DIST,
        MESO_CIRC_ID,
        MESO_SR,
        MESO_RV,
        MESO_DV,
    ...
    public enum DHRValues {
        ...
        CLUTTERTHRESHOLD,
        WEIGHTTHRESHOLD,
        FULLHYBRIDSCANTHRESH,
        LOWREFLTHRESHOLD,
        RAINDETREFLTHRESHOLD,
        RAINDETAREATHRESHOLD,
        RAINDETTIMETHRESHOLD,
        ZRMULTCOEFF,
        ZRPOWERCOEFF,
        MINREFLTORATE,
        MAXREFLTORATE,
     ...
     */
    public static final String MESO_REGEX = "\\s*(\\w+)\\s+(.{3}/.{3})\\s+(\\w+)\\s+(.{2})\\s+(.{3})\\s+(.{3})\\s+(.{3})\\s+(.{3})\\s+(\\w+)\\s+(\\w+)\\s+(\\w+)\\s+(\\w+)\\s\\s(.{7})\\s+(.{5})";
    ...
    public static final Pattern graphic_block_pattern = Pattern
            .compile(GRAPHIC_BLOCK, Pattern.CASE_INSENSITIVE);

    public static final Pattern hail_headerPattern = Pattern
            .compile(HAIL_HEADER, Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);

    public static final Pattern hail_productValues = Pattern.compile(HAIL_REGEX,
    ...
    public static final Pattern meso_headerPattern = Pattern
            .compile(MESO_HEADER, Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    ...
    public static final Pattern meso_productValues = Pattern.compile(MESO_REGEX,
            Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    ...
    public static final List<String> modeOfOperation = Collections
            .unmodifiableList(Arrays.asList("Maintenance Mode",
                    "Clear Air Mode", "Precipitation/Severe Weather Mode"));

    public static final List<String> rdaOpStatusStr = Collections
            .unmodifiableList(
                    Arrays.asList(null, "Online", "Maintenance Action Required",
                            "Maintenance Action Mandatory", "Command Shutdown",
                            "Inoperable", null, "Wideband Disconnect"));

    public static final List<String> rdaStatusStr = Collections
            .unmodifiableList(Arrays.asList(null, "Startup", "Standby",
                    "Restart", "Operate", null, "Off-line Operate"));

    public static final List<String> rdaAlarmStr = Collections
            .unmodifiableList(Arrays.asList("Indeterminate", "Tower/Utilities",
                    "Pedestal", "Transmitter", "Receiver", "RDA Control",
                    "RDA Communications", "Signal Processor"));

    public static final List<String> dteStr = Collections
            .unmodifiableList(Arrays.asList(null, "None", "Reflectivity",
                    "Velocity", "Spectrum Width", "Dual Pol Data Expected"));

    public static final List<String> rpgOpStr = Collections.unmodifiableList(
            Arrays.asList("Loadshed", "Online", "Maintenance Action Required",
                    "Maintenance Action Mandatory", "Commanded Shutdown"));

    public static final List<String> rpgAlarmStr = Collections.unmodifiableList(
            Arrays.asList("No Alarms", "Node Connectivity", "Wideband Failure",
                    "RPG Control Task Failure", "Data Base Failure", null,
                    "RPG Input Buffer Loadshed (Wideband)", null,
                    "Product Storage Loadshed", null, null, "Backup Comms",
                    "RPG/RPG Intercomputer Link Failure",
                    "Redundant Channel Error", "Task Failure",
                    "Media Failure"));

    public static final List<String> rpgStatusStr = Collections
            .unmodifiableList(Arrays.asList("Restart", "Operate", "Standby"));

    public static final List<String> productAvailStr = Collections
            .unmodifiableList(Arrays.asList("Products Available",
                    "Products Available but Degraded",
                    "Products Not Available"));

    public static final List<String> requestReponseErrorCode = Collections
            .unmodifiableList(Arrays.asList("No Such Message Code",
                    "No Such Product Code",
                    "Product Not Generated (Not Available in Data Base",
                    "One-Time Request Generation Process Faulted",
                    "Narrowband Loadshed", "Illegal Request",
                    "RPG Memory Loadshed", "RPG CPU Loadshed",
                    ...
                    "Task Failure (Self-Terminated)", "Command Not Authorized",
                    "Command Rejected"));

    public static final List<String> rpgNarrowbandStatus = Collections
            .unmodifiableList(Arrays.asList("Commanded Disconnect",
                    "Product Loadshedding Active"));

    public static final List<String> rdaChannelStr = Collections
            .unmodifiableList(
                    Arrays.asList("NWS Single Thread", "RDA 1", "RDA 2"));

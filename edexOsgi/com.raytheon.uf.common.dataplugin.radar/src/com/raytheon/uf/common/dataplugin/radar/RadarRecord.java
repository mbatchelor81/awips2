 ...
 * Nov 02, 2016  19447    D. Friedman  Call getDecodedTresholds() in
 *                                     isExpandedMode.
 * Mar 26, 2018  6711     randerso     Added GET_GSMS_BY_ID named query. Code
 *                                     cleanup.
 ...
 */
@NamedQueries({
        @NamedQuery(name = RadarRecord.GET_LATEST_RADAR_RECORDS, query = RadarRecord.GET_LATEST_RADAR_RECORDS_QUERY) })

@Entity
@SequenceGenerator(initialValue = 1, name = PluginDataObject.ID_GEN, sequenceName = "radarseq")
@Table(name = RadarRecord.PLUGIN_NAME, uniqueConstraints = {
        @UniqueConstraint(columnNames = { "dataURI" }) })
/*
 * Both refTime and forecastTime are included in the refTimeIndex since
 * forecastTime is unlikely to be used.
 */
@org.hibernate.annotations.Table(appliesTo = RadarRecord.PLUGIN_NAME, indexes = {
        @Index(name = "radar_refTimeIndex", columnNames = { "refTime",
                "forecastTime" }) })
@XmlRootElement
@XmlAccessorType(XmlAccessType.NONE)
@DynamicSerialize
public class RadarRecord extends PersistablePluginDataObject
        implements ISpatialEnabled, IRadarRecord {
    private static final Logger logger = LoggerFactory
            .getLogger(RadarRecord.class);

    private static final long serialVersionUID = 1L;

    public static final String GET_LATEST_RADAR_RECORDS = "getLatestRadarRecords";

    public static final String GET_LATEST_RADAR_RECORDS_QUERY = "FROM RadarRecord r WHERE r.icao = :icao AND r.productCode = :productCode ORDER BY r.dataTime.refTime DESC";
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    public Unit<?> getDataUnit() {
        ...
        if (numLevels <= 16) {
            ...
            if (allZeroes) {
                // allZeroes is not a valid unit and is basically discarded,
            ...
            double scale = Float
                    .intBitsToFloat((getThreshold(0) << 16) + getThreshold(1));
            if (scale == 0.0) {
            ...
            double offset = Float
                    .intBitsToFloat((getThreshold(2) << 16) + getThreshold(3));
     ...
     *            the cpmBlock to set
     */
    public void setCpmMessage(CPMMessage cpmMessage) {
        getStoredData().setCpmMessage(cpmMessage);
    }
     ...
     * @return the cpmBlock
     */
    public CPMMessage getCpmMessage() {
        return getStoredData().getCpmMessage();
    }
    ...
    public void setProductVals(
            Map<RadarConstants.MapValues, Map<String, Map<RadarConstants.MapValues, String>>> map) {
    ...
    private <T> void addPacketData(double i, double j, int type,
        ...
        if (needToConvert) {
            ...
            /*
             * for MIGFA and AMDA, i/j unit is 1km, for other radar products,
             * unit is 1/4km
             */
    ...
    public void correlateSymbologyPackets() {
        ...
        for (Layer currLayer : this.getSymbologyBlock().getLayers()) {
            ...
            for (int k = 0; k < layerPackets.length; k++) {
                ...
                if (currPacket instanceof TextSymbolPacket) {
                    ...
                    if (packet.getTheText().contains("\"")) {
                        ...
                        if (((k + 1) < layerPackets.length) && (layerPackets[k
                                + 1] instanceof StormIDPacket)) {
                        ...
                        if (((k + 2) < layerPackets.length) && (layerPackets[k
                                + 2] instanceof SCITDataPacket)) {
                            SCITDataPacket scitPktPast = (SCITDataPacket) layerPackets[k
                                    + 2];
                            addPacketData(i, j, type, RadarProductType.STI,
                            ...
                            if (((k + 3) < layerPackets.length)
                                    && (layerPackets[k
                                            + 3] instanceof SCITDataPacket)) {
                                SCITDataPacket scitPktFst = (SCITDataPacket) layerPackets[k
                                        + 3];
                                addPacketData(i, j, type, RadarProductType.STI,
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
                    ...
                    for (GenericDataComponent currComponent : packet
                        ...
                        for (int kk = 0; kk < numP; kk++) {
                            x = currFeature.getPoints().get(kk)
                                    .getCoordinate1();
                            y = currFeature.getPoints().get(kk)
                                    .getCoordinate2();
                            totalX += x;
                            totalY += y;
                        }
                        ...
                        addPacketData(centerX, centerY, type,
                                RadarProductType.GENERIC, currFeature,
                                convertLatLon);
                    }
                    continue;
     ...
     *
     * @param type
     * @return the ids of the specified type
     */
    public List<String> getIds(RadarConstants.MapValues type) {
        List<String> list = null;

        Map<MapValues, Map<String, Map<MapValues, String>>> mapProductVals = getMapProductVals();
        if (mapProductVals != null) {
            Map<String, Map<MapValues, String>> map = mapProductVals.get(type);
            if (map != null) {
    ...
    public boolean isExpandedMode() {
        boolean rval = false;
        Object threshold = getDecodedThresholds()[1];
...
>>>>>>> 3a1a5c9814b49f276bea4ebd9e584974d6ea7a11
    ...
    /**
     * Sets the delta time of the RadarRecord to the time calculated in
     * Level3BaseRadar, called by RadarDecoder
     */
    public void setDeltaTime(short time) {
        this.deltaTime = time;
    }

    /**
     * Sets the scan type of the RadarRecord as determined in Level3BaseRadar,
     * called by RadarDecoder
     */
    ...
    /**
     * Returns whether scan cut is a normal cut or a defined special cut
     *
     * @return the type of scan
     */
    ...
    /**
     * Returns the delta time of the scan cut Time is measured from the start of
     * the full volume scan to the start of the cut for which it was called (e.g
     * time from start of full scan [first cut] to start of cut 4 or 5 or 9
     * etc.)
     *

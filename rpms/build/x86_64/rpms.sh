...
function buildRPM()
{
   ...
   if [ $? -ne 0 ]; then
      ...
      exit 1
        ...
        #Tiffany added the line below for when she was initially trying to build the new ade docker image so that the rpms that were just built could be installed (since future rpms were dependent on them)
        #yum localinstall /awips2/jenkins/buildspace/workspace/tmp/rpms_built_dir/RPMS/*/${1}* -y
   return 0
}
...
function buildRPMExec()
{
   ...
   # Ignore return code as FOSS may call this function before
   #  the Eclipse RPM is created.
   buildTargetPlatform
...
function buildTargetPlatform()
{
   # If the TP already exists exit out of this function.
   if [ -d ${UFRAME_TARGET} ]; then
      return 0
   fi

   # Check if Eclipse is installed before trying to build the TP.
   #  If not then just exit out and skip for now.
   if [ ! -d /awips2/eclipse ]; then
      return -1
   fi
   # Same check but account for a broken link.
   if [[ -L /awips2/eclipse ]] && [[ ! -a /awips2/eclipse ]];then 
      return -1
   fi

   /awips2/python/bin/python3 ${WORKSPACE}/build/extractTarget.py \
      ${WORKSPACE}/build/eclipse.target \
      ${UFRAME_TARGET}
   return $?
}
...
function buildEDEX()
{
   buildTargetPlatform
   if [ $? -ne 0 ]; then
      echo "ERROR: Failed to build Eclipse Target Platform."
      exit 1
   fi

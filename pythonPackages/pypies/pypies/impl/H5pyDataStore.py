...
# Date          Ticket#  Engineer       Description
# ------------- -------- -------------- ----------------------------------------
# Jun 16, 2010           njensen        Initial Creation.
# May 03, 2011  9134     njensen        Optimized for pointdata
# Oct 09, 2012           rjpeter        Optimized __getGroup for retrievals
# Jan 17, 2013  15294    D. Friedman    Clear out data in response
# Feb 12, 2013  1608     randerso       Added support for explicitly deleting
#                                       groups and datasets
# Nov 14, 2013  2393     bclement       removed interpolation
# Jan 17, 2014  2688     bclement       added file action and subprocess error
#                                       logging
# Mar 19, 2014  2688     bgonzale       added more subprocess logging.  Return
#                                       value from subprocess.check_output is
#                                       not return code, but is process output.
#                                       h5repack has no output without -v arg.
# Apr 24, 2015  4425     nabowle        Add DoubleDataRecord
# Jun 15, 2015  17556    mgamazaychikov Add __doMakeReadable method to
#                                       counteract umask 027 daemon and make
#                                       copied files world-readable
# Jul 27, 2015  4402     njensen        Set fill_time_never on write if fill
#                                       value is None
# Jul 30, 2015  1574     nabowle        Add deleteOrphanFiles()
# Aug 20, 2015  17726    mgamazaychikov Remove __doMakeReadable method
# Sep 14, 2015  4868     rjpeter        Updated writePartialHDFData to create
#                                       the dataset if it doesn't exist.
# Oct 20, 2015  4982     nabowle        Verify datatypes match when replacing
#                                       data.
# Feb 15, 2016  3857     tgurney        Handle lowercase compression type
#                                       (e.g. 'lzf')
# Feb 24, 2016  5389     nabowle        Orphan purging now uses subdirectory
#                                       patterns with separate reftimes, if
#                                       applicable.
# Feb 26, 2016  5420     tgurney       Store and check for time of last repack
# Feb 29, 2016  5420     tgurney        Remove timestampCheck arg from copy()
# Apr 22, 2016  5389     nabowle        Fix purging false-orphans.
# Nov 15, 2016  5992     bsteffen       Support storing compressed records.
# Dec 22, 2016  19477    mporricelli    Check that file exists in copy() before
#                                       attempting file action
# May 05, 2017  6256     tgurney        Set restricted file permissions on copy()
# May 12, 2017  6256     tgurney        Set restricted file permissions on repack
# Jun 14, 2017  6256     tgurney        Set permissions on correct files (bug fix)
# Aug 16, 2018  6720     njensen        Improved __getNode() error message,
#                                       removed __getGroup()
# Sep 14, 2018  6448     dlovely        Fixed issue with h5repack and LZF
#                                       compression
# Sep 19, 2018  7435     ksunil         Eliminate compression/decompression on
#                                       HDF5
# Oct 11, 2018  7306     dgilling       Force getDatasets to always return
#                                       ndarray of type numpy.string_.
# May 15, 2019  7847     dlovely        Fix for segmentation fault using LZF
#                                       compression
# Jun 25, 2019  7885     tgurney        Python 3 fixes
# Jul 23, 2019  7885     tgurney        Decode h5repack subprocess output
# Aug 13, 2019  7885     tgurney        Fix str/bytes issue with delete requests
# Jan 28, 2020  7985     ksunil         Removed the compression changes
#                                       introduced in 7435
# Feb 11, 2020  7628     bsteffen       Ensure __retrieve only returns datasets.
#    Jan 17, 2022       8729       rjpeter        Call method cleanUp
# Jan 22, 2021  8321     dlovely        Migrate to GZIP compression
# Mar 30, 2021  8335     mapeters       Remove LZF compression
# Mar 30, 2021, 8374     randerso       Make getDatasets() return
...
=======
    ...
    def copy(self, request):
        ...
        if os.path.exists(filename):
            ...
            self.__doFileAction(filename, pth, request.getOutputDir(), action, resp, request.getRepackCompression())
        else:
            actionName = self.__getFileActionName(action)
